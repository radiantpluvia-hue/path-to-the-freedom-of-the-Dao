import { RandomMission } from './src/types';

interface MissionTemplate {
  type: RandomMission['type'];
  title: string; // e.g., "Defeat the {target} in the {location}"
  description: string; // e.g., "A group of {target} have been harassing travelers in the {location}. The sect requires them to be dealt with."
  targets: string[];
  locations: string[];
  reward: {
    spiritStones: { low: [number, number]; mid: [number, number] }; // min/max range
    sectReputation: [number, number];
    karma?: [number, number];
  };
}
