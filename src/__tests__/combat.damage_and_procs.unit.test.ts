...
describe('CombatSystem damage math and proc ordering', () => {
  test('miss occurs when RNG roll < missChance', () => {
    const attacker: any = { id: 'att', name: 'Attacker', hp: 50, maxHp: 50, qi: 10, maxQi: 10, ap: 5, maxAp: 5, stats: { atk: 10, def: 0, speed: 3 }, techniques: [], buffs: [], debuffs: [] };
    const target: any = { id: 'tgt', name: 'Target', hp: 40, maxHp: 40, qi: 0, maxQi: 0, ap: 5, maxAp: 5, stats: { atk: 5, def: 0, speed: 2 }, techniques: [], buffs: [], debuffs: [] };

    // RNG returns 0.01 which is below default missChance (0.05)
    const cs = new CombatSystem(attacker, [target], null, null, { type: 'normal', enableCritMiss: true, rng: () => 0.01 });
  ...
  test('crit deals strictly more damage than non-crit (same inputs)', () => {
    const makeParticipant = () => ({ id: 'p', name: 'P', hp: 100, maxHp: 100, qi: 0, maxQi: 0, ap: 5, maxAp: 5, stats: { atk: 8, def: 2, speed: 3 }, techniques: [], buffs: [], debuffs: [] } as any);

    const atkA = makeParticipant();
    const tgtA = makeParticipant();
    const csNormal = new CombatSystem(atkA, [tgtA], null, null, { type: 'normal', enableCritMiss: true, rng: () => 0.5 });
    const preA = tgtA.hp;
    ...
    const csCrit = new CombatSystem(atkB, [tgtB], null, null, { type: 'normal', enableCritMiss: true, rng: () => 0.99 });
    const preB = tgtB.hp;
  ...
  test('attacker onHit hook that returns extra damage is applied after main damage', () => {
    const attacker: any = { id: 'att', name: 'Attacker', hp: 30, maxHp: 30, qi: 0, maxQi: 0, ap: 5, maxAp: 5, stats: { atk: 5, def: 0, speed: 3 }, techniques: [], buffs: [], debuffs: [] };
    const target: any = { id: 'tgt', name: 'Target', hp: 20, maxHp: 20, qi: 0, maxQi: 0, ap: 5, maxAp: 5, stats: { atk: 3, def: 0, speed: 2 }, techniques: [], buffs: [], debuffs: [] };

    // Attach onHit passive that returns 3 extra damage
    attacker._passiveHooks = { onHit: [{ id: 'test_extra', fn: ({ attacker, target }: any) => { return 3; } }] };
