...
test('CombatSystem.notifyEnemyDefeated updates mission objectives', () => {
  ...
  mockUseGameStore({ player: { sect: 's1', inventory: [] }, story, updateMissionObjectiveProgress, getSectReputation });
  ...
  const cs = new Combat(player as any, [enemy as any], require('../store/useGameStore').useGameStore().getState?.() || require('../store/useGameStore').useGameStore());
...
test('CraftingSystem.notifyItemCrafted updates mission objectives', () => {
  ...
  mockUseGameStore({ player: { sect: 's1', inventory: [] }, story, updateMissionObjectiveProgress, getSectReputation: getSectReputation2 });
