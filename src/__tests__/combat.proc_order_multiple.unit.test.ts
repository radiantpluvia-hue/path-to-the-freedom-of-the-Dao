...
describe('CombatSystem passive/proc ordering with multiple hooks and RNG-driven procs', () => {
  test('multiple onHit and proc hooks execute in expected order and RNG affects proc invocation', () => {
    ...
    const tech: any = { id: 'atk', name: 'Strike', apCost: 0, qiCost: 0, type: 'attack', effects: [{ type: 'damage', target: 'enemy', value: 6 }] };
    ...
    (attacker as any)._passiveHooks = {
      ...
      proc: [
        ...
        { id: 'a_proc_1', fn: ({ rng }: any) => { if (rng() < 0.2) { log.push('a_proc_1'); } } },
        // Proc that triggers only if rng() < 0.9
        { id: 'a_proc_2', fn: ({ rng }: any) => { if (rng() < 0.9) { log.push('a_proc_2'); } } }
      ]
    };
