...
describe('HeavenlyDaoSystem reversal techniques', () => {
  it('applies Reverse Pulse Sutra healing or backlash', () => {
    ...
    const s = new HeavenlyDaoSystem(engine);
    const player: any = { hp: 40, maxHp: 100, daoHeart: 10, daoComprehension: 0, activeBuffs: [] };
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if (res.ok && res.healed) sawSuccess = true;
      if (!res.ok && res.backlash) sawFail = true;
      if (sawSuccess && sawFail) break;
    }
  ...
  it('applies Still Flow Meditation buff and increases comprehension', () => {
    const engine = new NarrativeEngine();
    const s = new HeavenlyDaoSystem(engine);
    const player: any = { hp: 80, maxHp: 120, daoHeart: 20, daoComprehension: 3, activeBuffs: [] };
    ...
    expect(player.activeBuffs.find((b: any) => b.id === 'still_flow_meditation')).toBeTruthy();
  });

  it('consumes dao heart for Temporal Echo', () => {
    const engine = new NarrativeEngine();
    const s = new HeavenlyDaoSystem(engine);
    const player: any = { hp: 50, maxHp: 100, daoHeart: 10, daoComprehension: 0 };
