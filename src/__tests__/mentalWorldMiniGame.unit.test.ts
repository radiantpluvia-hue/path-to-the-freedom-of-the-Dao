import { runMentalWorldMiniGame } from '@/systems/mentalWorldMiniGame';
import { createSampleConsciousnessFor } from '@/systems/immortalEmperorConsciousness';

describe('Mental world mini-game', () => {
  test('passing mini-game returns rewardPower', () => {
    const res = runMentalWorldMiniGame(200, { humility: true, recallHint: true }, 1);
    expect(res.success).toBe(true);
    expect(res.rewardPower).toBeGreaterThan(0);
  });

  test('failing mini-game returns backlash', () => {
    const res = runMentalWorldMiniGame(10, { humility: false }, 1);
    expect(res.success).toBe(false);
    expect(res.failureBacklash).toBeGreaterThan(0);
  });

  test('integration: consciousness reacts to mini-game result', () => {
    const c = createSampleConsciousnessFor('Integration Emperor', 'Test Era');
    const res = runMentalWorldMiniGame(200, { humility: true }, 1);
    if (res.success) {
