...
const fn_act1_tutorial_welcome: EventExecutor = (state) => {
  const s: any = {
    ...state,
    player: {
      ...state.player,
  ...
  const existingManuals = s.player.manuals as any[];
  ...
  // record event run
  (s.__meta ||= {}).eventsRan = (s.__meta.eventsRan || []);
  if (!s.__meta.eventsRan.includes('fn_act1_tutorial_welcome')) s.__meta.eventsRan.push('fn_act1_tutorial_welcome');
  return s as GameState;
};

const fn_act1_first_meditation: EventExecutor = (state) => {
  const s: any = {
    ...state,
  ...
  (s.__meta ||= {}).eventsRan = (s.__meta.eventsRan || []);
  if (!s.__meta.eventsRan.includes('fn_act1_first_meditation')) s.__meta.eventsRan.push('fn_act1_first_meditation');
  return s as GameState;
};

const fn_act1_combat_intro: EventExecutor = (state) => {
  const s: any = {
    ...state,
  ...
  const existing = s.player.inventory.find((i: any) => i && i.id === 'training_dummy');
  if (!existing) s.player.inventory.push({ id: 'training_dummy', name: 'Training Dummy', qty: 1 });
  (s.__meta ||= {}).eventsRan = (s.__meta.eventsRan || []);
  if (!s.__meta.eventsRan.includes('fn_act1_combat_intro')) s.__meta.eventsRan.push('fn_act1_combat_intro');
  return s as GameState;
};

const fn_act1_cultivation_awakening: EventExecutor = (state) => {
  ...
  if (typeof s.player.level === 'number') s.player.level = Math.max(s.player.level, 1);
  s.player.daoComprehension = (s.player.daoComprehension || 0) + 1;
  s.world.flags.cultivation_awakening = true;
  (s.__meta ||= {}).eventsRan = (s.__meta.eventsRan || []);
  if (!s.__meta.eventsRan.includes('fn_act1_cultivation_awakening')) s.__meta.eventsRan.push('fn_act1_cultivation_awakening');
  return s as GameState;
};

const fn_act1_sect_entrance: EventExecutor = (state) => {
  const s: any = {
    ...state,
  ...
  (s.__meta ||= {}).eventsRan = (s.__meta.eventsRan || []);
  if (!s.__meta.eventsRan.includes('fn_act1_sect_entrance')) s.__meta.eventsRan.push('fn_act1_sect_entrance');
  return s as GameState;
};

const fn_act1_spirit_stone: EventExecutor = (state) => {
  ...
  (s.__meta ||= {}).eventsRan = (s.__meta.eventsRan || []);
  if (!s.__meta.eventsRan.includes('fn_act1_spirit_stone')) s.__meta.eventsRan.push('fn_act1_spirit_stone');
  return s as GameState;
};
...
export const act1EventExecutors: Record<string, EventExecutor> = {
