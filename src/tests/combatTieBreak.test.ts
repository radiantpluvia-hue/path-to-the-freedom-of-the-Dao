...
import { computeOffensiveProwess } from '../systems/combatConfig';
...
describe('CombatSystem turn-order tie-breaker parity', () => {
  test('player goes first on identical speed and prowess', () => {
    ...
    const cs = new (CombatSystem as any)(player, [enemy], null, null, { type: 'normal' });
  ...
  test('computeOffensiveProwess matches inline heuristic', () => {
    ...
    expect(computeOffensiveProwess(e)).toBe(inline);
  });
});
