...
import { RARITY_CONFIG, getEnhancedPassiveLevelsFor } from '../config/rarity';
...
const RACE_BACKGROUNDS_RAW: Record<string, Background[]> = {
  Human: [
    {
      id: 'human_noble',
      name: 'Noble Lineage',
...
function withComputedStartChances(input: Record<string, Background[]>): Record<string, Background[]> {
  const out: Record<string, Background[]> = {};
  for (const race of Object.keys(input)) {
    const arr = input[race];
    const counts: Record<string, number> = {};
    for (const bg of arr) {
      if (bg.startChance === undefined) counts[bg.rarity] = (counts[bg.rarity] || 0) + 1;
    }
    out[race] = arr.map((bg) => {
      if (typeof bg.startChance === 'number') return bg;
      const cfg = RARITY_CONFIG[bg.rarity as keyof typeof RARITY_CONFIG];
...
function withEnhancedPassives(input: Record<string, Background[]>): Record<string, Background[]> {
  const out: Record<string, Background[]> = {};
  for (const race of Object.keys(input)) {
    out[race] = input[race].map((bg) => {
  const upgrades = getEnhancedPassiveLevelsFor(bg.rarity);
      if (!upgrades) return bg;
      const effects = (bg.effects || {}) as any;
      const passives = effects.passives || {};
      let changed = false;
      const upgraded: Record<string, any> = { ...passives };
      ...
      if (Object.keys(passives).length === 0) {
        const rarity = (bg.rarity || "H") as string;
        // default counts: legendary 4, rare 3, uncommon 2, common 1
        const defaultCountMap: Record<string, number> = { legendary: 4, rare: 3, uncommon: 2, common: 1 };
        let count = defaultCountMap[rarity] || 1;
        // Humans are given fewer passives (1-3)
        if (race.toLowerCase() === 'human') count = Math.min(3, count);
        const gen: Record<string, any> = {};
        for (let i = 1; i <= count; i++) {
          const pid = `${race.toLowerCase()}_${bg.id}_passive_${i}`;
          gen[pid] = { level: 1 };
        }
        return { ...bg, effects: { ...effects, passives: gen } } as Background;
      }

      if (!changed) return bg;
      return { ...bg, effects: { ...effects, passives: upgraded } } as Background;
    });
  }
  return out;
}

export const RACE_BACKGROUNDS: Record<string, Background[]> = withEnhancedPassives(withComputedStartChances(RACE_BACKGROUNDS_RAW));
...
function withLuckLabels(input: Record<string, Background[]>): Record<string, Background[]> {
  const out: Record<string, Background[]> = {};
  for (const race of Object.keys(input)) {
    out[race] = input[race].map((bg) => {
      const raw = bg.rarity || 'H';
      // Normalize to canonical tier letter (H/G/F/E/D/B) when possible
      const tier = migrateTier(String(raw));
      // Map letters to roughly-centered index where H -> Normal (3)
      const letterToIndex: Record<string, number> = { H: 3, G: 4, F: 5, E: 6, D: 7, B: 7 };
      const idx = letterToIndex[tier.replace(/[+-]$/, '')] ?? 3;
      return { ...bg, luckIndex: Math.max(0, Math.min(LUCK_LABELS.length - 1, idx)), luckLabel: LUCK_LABELS[Math.max(0, Math.min(LUCK_LABELS.length - 1, idx))] } as Background & { luckIndex: number; luckLabel: string };
    });
  }
  return out;
}
...
export const RACE_BACKGROUNDS_WITH_LUCK = withLuckLabels(RACE_BACKGROUNDS);