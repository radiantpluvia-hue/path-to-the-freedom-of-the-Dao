...
const BASE_REALMS: Record<string, CultivationRealm> = {
  mortal: {
...
export const CULTIVATION_REALMS: Record<string, CultivationRealm> = {} as Record<string, CultivationRealm>;
...
(() => {
  const suffixScale: Record<string, number> = { early: 0.65, mid: 0.85, late: 1.0, peak: 1.25 };
  // Lifespan overrides provided by the user (finalized list). These are absolute values.
  const LIFESPAN_OVERRIDES: Record<string, number> = {
    mortal: 100,
    qi_refinement: 300,
  ...
  const QI_OVERRIDES: Record<string, number> = {
    mortal: 100,
    qi_refinement: 300,
    foundation_establishment: 500,
    core_formation: 1000,
  ...
  const MINOR_STAGE_OVERRIDES: Record<string, number> = {
    qi_refinement: 12,
  ...
  const POST_ZENITH_SUFFIX_OVERRIDES: Record<string, number> = {
  ...
  const zenithPeakIndex = REALM_ORDER.indexOf('zenith_heaven_peak');

  for (let i = 0; i < REALM_ORDER.length; i++) {
    const id = REALM_ORDER[i];
    if ((CULTIVATION_REALMS as any)[id]) continue;

    // Use explicit base if available
    if (BASE_REALMS[id]) {
      const entry = { ...BASE_REALMS[id] } as CultivationRealm;
      ...
      (CULTIVATION_REALMS as any)[id] = entry;
      continue;
    }
    ...
    let base = BASE_REALMS[baseId] || (CULTIVATION_REALMS as any)[baseId];

    // If not found, search backwards
    if (!base) {
      for (let j = i - 1; j >= 0; j--) {
        const cand = REALM_ORDER[j];
        base = BASE_REALMS[cand] || (CULTIVATION_REALMS as any)[cand];
        if (base) break;
      }
    }
  ...
  const qiRequirement = QI_OVERRIDES[id] !== undefined ? QI_OVERRIDES[id] : (base ? Math.max(1, Math.floor(base.qiRequirement * scale)) : 1000);
    ...
    (CULTIVATION_REALMS as any)[id] = {
      ...
      nextRealm: REALM_ORDER[i + 1]
    } as CultivationRealm;
  }
})();

// Build a stable map of qi requirements for quick lookups. Populate after CULTIVATION_REALMS is ready.
export const REALM_QI_REQUIREMENTS: Record<string, number> = Object.fromEntries(
  Object.entries(CULTIVATION_REALMS).map(([key, realm]) => [key, realm.qiRequirement])
);


export function getNextRealm(currentRealm: string): string | undefined {
  ...
  const explicit = CULTIVATION_REALMS[currentRealm];
  if (explicit && explicit.nextRealm) return explicit.nextRealm;
  const idx = REALM_ORDER.indexOf(currentRealm);
  if (idx >= 0 && idx + 1 < REALM_ORDER.length) return REALM_ORDER[idx + 1];
  return undefined;
}

export function getRealmBreakthroughDifficulty(currentRealm: string): number {
  return CULTIVATION_REALMS[currentRealm]?.breakthroughDifficulty || 0;
}

export function getRealmLifespanBonus(currentRealm: string): number {
  return CULTIVATION_REALMS[currentRealm]?.lifespanBonus || 0;
}

export function getMinorStageCount(realm: string): number {
  return CULTIVATION_REALMS[realm]?.minorStages || 1;
}
