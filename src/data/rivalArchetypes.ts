...
export const RIVAL_ARCHETYPES: Record<string, RivalArchetype> = {
  young_prodigy: {
    id: 'young_prodigy',
    name: 'Young Prodigy',
...
export const getArchetypeById = (id: string): RivalArchetype | null => {
  return RIVAL_ARCHETYPES[id] || null;
};

export const getRandomArchetype = (factionBias?: string[], sectBias?: string[]): RivalArchetype | null => {
  ...
  if (factionBias?.length) {
    filteredArchetypes = archetypes.filter(archetype =>
      archetype.factionBias.some(faction => factionBias.includes(faction))
    );
  }
  if (sectBias?.length) {
    filteredArchetypes = filteredArchetypes.filter(archetype =>
      archetype.sectBias.some(sect => sectBias.includes(sect))
    );
  }
...
export const getArchetypesByPersonality = (personality: Rival['personality']): RivalArchetype[] => {
  return Object.values(RIVAL_ARCHETYPES).filter(archetype => archetype.personality === personality);
};
