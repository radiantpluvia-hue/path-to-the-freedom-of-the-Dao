// Hidden encounters / one-off NPCs
import { RivalEncounter } from '../types';

export interface HiddenEncounterDef {
  id: string;
  name: string;
  description: string;
  spawnCondition?: (gameState: any) => boolean;
  chance?: number; // 0..1 per check
  createEncounter?: (gameState: any) => RivalEncounter;
}

export const KID_GOD: HiddenEncounterDef = {
  id: 'kid_god_taek_jin',
  name: 'Kid God (Taek Jin)',
  description:
    "A reincarnation of Sun Wukong, born from Heaven and Earth's spiritual energy. Trained by his mortal grandfather in Recoilless Taekwondo, he wields the Ruyi Jingu Bang and seeks to challenge the Heavenly Demonic forces.",
  ...
  createEncounter: (gameState: any) => {
    ...
    return {
      ...
      description:
        "Kid God appears, a heaven-defying genius armed with the Ruyi Jingu Bang and perfected kicks. Prepare for a legendary clash.",
      outcome: 'pending',
      lootGained: [],
      reputationChange: {},
...
export const HIDDEN_ENCOUNTERS: HiddenEncounterDef[] = [KID_GOD];
