...
export interface BuildingPrototype {
  ...
  yieldBonus?: Partial<Record<ResourceKey, number>>;
  defenseBonus?: number;
  ...
  slots?: number; // number of socket slots for upgrades
  maxLevel?: number;
}
...
export interface TerritoryType {
  ...
  name: string;
  description: string;
  baseYield: Partial<Record<ResourceKey, number>>;
  defenseModifier: number;
  buildableBuildings: string[]; // building prototype IDs
...
export interface FactionTemplate {
  ...
  name: string;
  description: string;
  startingRelation: 'ally' | 'neutral' | 'hostile';
  techTree: Record<string, FactionTech>;
  color?: string;
  icon?: string;
}
...
export interface FactionTech {
  ...
  effects: {
    yieldModifiers?: Partial<Record<ResourceKey, number>>;
    upkeepReduction?: number;
    exclusiveBuildings?: string[];
    territoryBonuses?: Record<string, any>;
  };
  prerequisites?: string[]; // other tech IDs required
  domainLevelRequired?: number;
}

export const BUILDING_PROTOTYPES: Record<string, BuildingPrototype> = {
  watchtower: {
    id: 'watchtower',
    name: 'Watchtower',
...
export const TERRITORY_TYPES: Record<string, TerritoryType> = {
  town: {
    id: 'town',
    name: 'Town',
...
export const FACTION_TEMPLATES: Record<string, FactionTemplate> = {
  player_faction: {
    id: 'player_faction',
    name: 'Player Domain',
