...
const GROUP_TO_TIER: Record<string, SkillTier> = {
  'Earth/Stone': 3,
  'Darkness/Demonic': 4,
  'Light/Heavenly': 5,
  'Yin-Yang': 5,
...
const CATEGORY_EFFECTS: Record<string, (name: string, power: number) => Skill['effects']> = {
...
const GROUPS: Array<{ name: string; names: string[] }> = [
...
export const MORTAL_IMPORTED_SKILLS: Skill[] = [];
export const IMMORTAL_IMPORTED_SKILLS: Skill[] = [];
...
for (const g of GROUPS) {
  ...
  for (let i = 0; i < g.names.length; i++) {
    ...
    if (tier <= 4) MORTAL_IMPORTED_SKILLS.push(skill);
    else IMMORTAL_IMPORTED_SKILLS.push(skill);
  }
}

export default {
  MORTAL_IMPORTED_SKILLS,
  IMMORTAL_IMPORTED_SKILLS,
};
