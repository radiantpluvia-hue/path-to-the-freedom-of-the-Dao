...
const TIER_QI_BASE: Record<number, number> = {
  1: 4,
  2: 12,
  3: 28,
  4: 60,
...
export default MORTAL_WORLD_SKILLS;

// Import user-generated mortal skills and merge (avoid duplicates by ID)
import USER_SKILLS from './user_generated_skills';
const existingIds = new Set(MORTAL_WORLD_SKILLS.map(s => s.id));
for (const s of USER_SKILLS.MORTAL_IMPORTED_SKILLS) {
  if (!existingIds.has(s.id)) {
    MORTAL_WORLD_SKILLS.push(s);
