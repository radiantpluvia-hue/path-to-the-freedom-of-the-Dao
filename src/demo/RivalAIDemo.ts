import { RivalAISystem, CombatSystem } from '../systems';
import { Rival } from '../types';

// Demo script to test the Dynamic AI Behavior system
export class RivalAIDemo {
  private aiSystem: RivalAISystem;
  private mockCombatSystem: CombatSystem;

  constructor() {
    this.mockCombatSystem = {} as CombatSystem;
    this.aiSystem = new RivalAISystem(this.mockCombatSystem);
  }
  ...
  runDemo() {
    ...
    rivals.forEach(rival => {
      const decision = this.aiSystem.makeCombatDecision(rival, {} as any, combatSituation);
    ...
    for (let i = 0; i < 5; i++) {
      this.aiSystem.recordEncounterResult(
        testRival.id,
    ...
    const adaptedDecision = this.aiSystem.makeCombatDecision(testRival, {} as any, combatSituation);
    ...
    situations.forEach(situation => {
      const decision = this.aiSystem.makeCombatDecision(testRival, {} as any, {
        ...combatSituation,
        ...situation
      });
