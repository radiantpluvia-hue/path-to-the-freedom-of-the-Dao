...
export type RealmTier = 'early' | 'mid' | 'late' | 'peak';

// Map realm names or ids to tiers roughly. This is a conservative mapping; callers can override.
export function realmToTier(realm: string): RealmTier {
  const r = realm.toLowerCase();
  if (r.includes('mortal') || r.includes('mortal')) return 'early';
  if (r.includes('foundation') || r.includes('foundation')) return 'mid';
  if (r.includes('qi') || r.includes('qi condensation') || r.includes('condensation')) return 'mid';
  if (r.includes('core') || r.includes('core formation')) return 'late';
  if (r.includes('nascent') || r.includes('nascent soul') || r.includes('nascent')) return 'late';
  // fallback to early
  return 'early';
}

// Convert tier to multiplier for proficiency -> stat conversion
export function tierMultiplier(tier: RealmTier): number {
  switch (tier) {
...
export function addProficiency(state: GameState, stat: string, realm: string, points: number) {
...
export function computeStatFromProficiency(state: GameState, stat: string): number {
  const realm = getPlayerRealmKey(state.player);
