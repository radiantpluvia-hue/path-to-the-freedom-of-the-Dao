...
import getPowerScale from './powerScale';
...
export interface DerivedStats {
  ...
  maxQi: number;
  willPower: number;
  comprehension: number; // intelligence-like stat
}
...
export function computeDerivedStats(player: PlayerState): DerivedStats {
  const base = { ...(player.baseStats || {}), ...(player.stats || {}) } as Record<string, number>;
  ...
  const willFromField = typeof (player as any).willPower === 'number' ? (player as any).willPower : undefined;
  ...
  if (Array.isArray(player.activeBuffs)) {
    for (const b of player.activeBuffs) {
      ...
      if (typeof stats.willPower === 'number') finalWillPower += stats.willPower;
    }
  }
  ...
  try {
  const pathwayBonuses = ((player as any).cultivation && (player as any).cultivation.bonuses) || {};
    if (pathwayBonuses && Object.keys(pathwayBonuses).length > 0) {
      ...
      const applied = CultivationSystem.applyPathwayBonuses(player as any, derivedLike);
      ...
      for (const [k, v] of Object.entries(pathwayBonuses)) {
        ...
        if (key.endsWith('pct') || key.endsWith('%')) {
          ...
          if (baseKey.includes('speed')) finalSpeed = Math.round(finalSpeed + finalSpeed * val);
          else if (baseKey.includes('atk') || baseKey.includes('attack')) finalAtk = Math.round(finalAtk + finalAtk * val);
          else if (baseKey.includes('def')) finalDef = Math.round(finalDef + finalDef * val);
          else if (baseKey.includes('qi')) finalMaxQi = Math.round(finalMaxQi + finalMaxQi * val);
          else if (baseKey.includes('will')) finalWillPower = Math.round(finalWillPower + finalWillPower * val);
          else if (baseKey.includes('compreh') || baseKey.includes('percep')) finalComprehension = Math.round(finalComprehension + finalComprehension * val);
        }
      }
    }
  } catch (e) {
  ...
  try {
    const { statScale } = getPowerScale(player as any, (player as any)?.world || undefined);
...
export default computeDerivedStats;
