import { Physique, PhysiqueSynergy } from '@/types';

// Define physique synergy combinations
export const PHYSIQUE_SYNERGIES: PhysiqueSynergy[] = [
...
export function getActiveSynergies(activePhysiques: Physique[]): PhysiqueSynergy[] {
  const activePhysiqueIds = new Set(activePhysiques.map((p: Physique) => p.id));
  const activeSynergies: PhysiqueSynergy[] = [];

  for (const synergy of PHYSIQUE_SYNERGIES) {
    const hasAllRequired = synergy.requiredPhysiques.every((physiqueId: string) => activePhysiqueIds.has(physiqueId));
    if (hasAllRequired) {
...
export function calculateSynergyBonuses(activePhysiques: Physique[]): Record<string, number> {
  const activeSynergies = getActiveSynergies(activePhysiques);
  const totalBonuses: Record<string, number> = {};

  for (const synergy of activeSynergies) {
    for (const [bonusType, bonusValue] of Object.entries(synergy.bonuses as Record<string, number>)) {
...
export function getSynergyDescriptions(activePhysiques: Physique[]): string[] {
  const activeSynergies = getActiveSynergies(activePhysiques);
...
export function checkSynergyUnlock(physiqueIds: string[]): PhysiqueSynergy | null {
  for (const synergy of PHYSIQUE_SYNERGIES) {
    const hasAllRequired = synergy.requiredPhysiques.every((physiqueId: string) => physiqueIds.includes(physiqueId));
    if (hasAllRequired) {
      return synergy;
    }
  }
