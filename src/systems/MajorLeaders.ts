import type { Sect, Faction } from './SectSystem';
...
export const MAJOR_LEADERS: (Sect | Faction)[] = [
  {
    id: 'mount_hua_sect',
    name: 'Mount Hua Sect',
    description:
...
(() => {
  const normalizeCultivation = (input: any) => {
  ...
  for (const entry of MAJOR_LEADERS as any[]) {
    // cultivation
...
// Auto-adjust Bei Xue's age relative to Bei Ling (assumption: 10% younger).
(() => {
  const beiLing = MAJOR_LEADERS.find((e) => e.id === 'upper_north_sect') as any;
  const beiXue = MAJOR_LEADERS.find((e) => e.id === 'lower_south_sect') as any;
...
(() => {
  ...
  const normalizeKey = (cult: string) => String(cult || '').toLowerCase().replace(/[^a-z0-9_\s]/g, ' ').replace(/\s+/g, ' ').trim();

  const findBestMatch = (cult: string) => {
    if (!cult) return 'mortal';
    const nk = normalizeKey(cult);
    if (ALIASES[nk]) return ALIASES[nk];
    const underscore = nk.replace(/\s+/g, '_');
    if (CULTIVATION_REALMS[underscore]) return underscore;
    ...
    for (const key of realmKeys) {
      if (key === nk || key === underscore) return key;
    }
    return 'mortal';
  };
  ...
  for (const entry of MAJOR_LEADERS as any[]) {
    ...
    const idx = REALM_ORDER.indexOf(realmKey);
    entry.realm = idx >= 0 ? idx : REALM_ORDER.indexOf('mortal');
  }
})();
...
(() => {
  for (const entry of MAJOR_LEADERS as any[]) {
    const realmKey = entry.leaderRealm || REALM_ORDER[entry.realm] || 'mortal';
    const realmData = CULTIVATION_REALMS[realmKey];
