...
export type CultivationInfo = {
  stage?: number; // integer stage, 0 = mortal, higher = stronger realms
  substage?: number; // optional finer granularity (0-9)
};

// Compute a multiplier based on cultivation. Default: linear-ish scaling with soft cap.
export function getCultivationMultiplier(info?: CultivationInfo): number {
  if (!info || typeof info.stage !== 'number') return 1;
...
export function getDiminishingReturnFactor(useCount: number): number {
  if (!useCount || useCount <= 1) return 1;
...
export function adjustDamageForRealmGap(options: {
  attackerStage?: number;
  targetStage?: number;
  attackerProwess?: number; // derived metric (e.g., atk + spd)
  targetProwess?: number; // derived metric (e.g., def + stage*20)
  damage: number;
  targetMaxHp: number;
  minHpFraction?: number; // minimum fraction to leave if not allowed to kill
  gapThreshold?: number; // how many stages difference triggers protection
}): number {
