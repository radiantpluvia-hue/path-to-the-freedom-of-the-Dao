import type { EnhancedFormation } from '../data/formations';
...
export class NodeMapSystem {
  private static instance: NodeMapSystem;
  private mapWidth = 10;
  private mapHeight = 10;

  private constructor() { void 0; }

  static getInstance(): NodeMapSystem {
   ...
   */
  createFormationLayout(formation: EnhancedFormation, centerX = 5, centerY = 5): FormationLayout {
    const nodes: MapNode[] = [];
    // Defensive: formation.positions may be missing in some fixtures; fall back to a single-center position
  const positions: Array<{ x: number; y: number; bonuses?: any }> = formation.positions && formation.positions.length ? formation.positions as any : [{ x: 0, y: 0, bonuses: {} }];

  // Create nodes based on formation positions
  positions.forEach((pos: { x: number; y: number; bonuses?: any }, _index: number) => {
      const node: MapNode = {
        x: centerX + pos.x,
        y: centerY + pos.y,
        occupied: false,
   ...
   */
  assignUnitsToFormation(layout: FormationLayout, unitIds: string[]): FormationLayout {
   ...
   */
  calculatePositionalBonuses(layout: FormationLayout, unitId: string): {
    atk: number;
    def: number;
    speed: number;
    flankingBonus: number;
   ...
   */
  canFlank(layout: FormationLayout, attackerUnitId: string, targetUnitId: string): boolean {
    const attackerNode = layout.nodes.find(n => n.unitId === attackerUnitId);
    const targetNode = layout.nodes.find(n => n.unitId === targetUnitId);

    if (!attackerNode || !targetNode) return false;
   ...
   */
  isBackAttack(layout: FormationLayout, attackerUnitId: string, targetUnitId: string): boolean {
    const attackerNode = layout.nodes.find(n => n.unitId === attackerUnitId);
    const targetNode = layout.nodes.find(n => n.unitId === targetUnitId);
   ...
   */
  getGlobalBonuses(layout: FormationLayout): {
   ...
   */
  calculateCoordinationBonus(layout: FormationLayout): number {
   ...
   */
  canDeployFormation(formation: EnhancedFormation, centerX: number, centerY: number): boolean {
  const positions: Array<{ x: number; y: number }> = formation.positions && formation.positions.length ? formation.positions as any : [{ x: 0, y: 0 }];
  return positions.every((pos: { x: number; y: number }) => {
   ...
   */
  getValidDeploymentPositions(formation: EnhancedFormation): { x: number; y: number }[] {
   ...
   */
  rotateFormation(layout: FormationLayout, clockwise = true): FormationLayout {
   ...
   */
  getFormationEffectiveness(layout: FormationLayout): number {
