...
// Clean, permissive types to accept the richer data migrated from the built artifact.
export interface Sect {
  id: string;
  name: string;
  description?: string;
  realm?: number; // numeric index into REALM_ORDER
  // canonical leader realm key (prefer string keys into CULTIVATION_REALMS)
  leaderRealm?: string;
  type?: string;
  power?: number;
  reputation?: number;
  ...
  sectComposition?: any;
  // leader metadata (kept permissive)
  leaderName?: string;
  leaderTitle?: string;
  leaderDescription?: string;
  leaderAge?: number | string;
  leaderCultivation?: string;
  leaderTechniques?: string[];
  leaderSelfCreatedTechniques?: string[];
  leaderWeapon?: string;
  pills?: string[];
  [key: string]: any;
}

export interface Faction { id: string; name: string; description?: string; members?: string[]; services?: any[]; [key: string]: any }
export interface Alliance { id: string; name: string; members: string[]; [key: string]: any }
export interface FactionService { id: string; name: string; requirements?: any; [key: string]: any }
...
export const MAJOR_SECTS: Sect[] = [
  {
    id: 'mount_hua_sect',
    name: 'Mount Hua Sect',
    description:
      'Balance between swordsmanship and internal cultivation; purity of heart guides strength. Famous for the Plum Blossom sword style.',
...
const _TEN_GREAT_MARTIAL_LEADERS: Record<string, Partial<Sect>> = {
  diancang_sect: {
...
MAJOR_SECTS.forEach(s => {
  if (s && s.id && !Object.prototype.hasOwnProperty.call(_TEN_GREAT_MARTIAL_LEADERS, s.id)) return;
  const meta = _TEN_GREAT_MARTIAL_LEADERS[s.id as string];
  if (!meta) return;
  // Only set fields that are missing so we don't override intentionally authored data.
  if (!s.leaderName && meta.leaderName) s.leaderName = meta.leaderName;
  if (!s.leaderTitle && meta.leaderTitle) s.leaderTitle = meta.leaderTitle;
  if (!s.leaderDescription && meta.leaderDescription) s.leaderDescription = meta.leaderDescription;
});
...
const _MAJOR_SECT_LEADERS: Record<string, Partial<Sect>> = {
...
MAJOR_SECTS.forEach(s => {
  if (!s || !s.id) return;
  const meta = _MAJOR_SECT_LEADERS[s.id as string];
  if (!meta) return;
  if (!s.leaderName && meta.leaderName) s.leaderName = meta.leaderName;
  if (!s.leaderTitle && meta.leaderTitle) s.leaderTitle = meta.leaderTitle;
  if (!s.leaderDescription && meta.leaderDescription) s.leaderDescription = meta.leaderDescription;
...
function mapCultivationStringToRealmKey(cult?: string): string | undefined {
  if (!cult || typeof cult !== 'string') return undefined;
  const s = cult.toLowerCase();
  if (s.includes('mortal')) return 'mortal';
  if (s.includes('qi')) return 'qi_refinement';
  if (s.includes('foundation')) return 'foundation_establishment';
  if (s.includes('core')) return 'core_formation';
  if (s.includes('nascent')) return 'nascent_soul';
  if (s.includes('soul')) return 'soul_transformation';
  if (s.includes('void')) return 'emptiness_refinement';
  if (s.includes('body')) return 'body_integration';
  if (s.includes('tribulation')) return 'tribulation_transcendence';
  if (s.includes('mahayana')) return 'mahayana';
  if (s.includes('golden')) return 'golden_immortal';
  if (s.includes('taiyi')) return 'taiyi_golden_immortal';
  if (s.includes('daluo')) return 'daluo_golden_immortal';
  if (s.includes('loose immortal') || s.includes('loose')) return 'loose_immortal';
  if (s.includes('true heaven')) return 'true_heaven_immortal';
  if (s.includes('heaven')) return 'heaven_immortal';
  if (s.includes('mystic')) return 'mystic_immortal';
  if (s.includes('silver')) return 'silver_immortal';
  if (s.includes('perfect') && s.includes('emperor')) return 'perfect_immortal_emperor_rank1';
  if (s.includes('mystic divine')) return 'mystic_celestial_origin_1';
  if (s.includes('zenith') || s.includes('quasi-sage') || s.includes('sage') || s.includes('saint') || s.includes('pseudo') || s.includes('perfected') || s.includes('freedom') || s.includes('pseudo') || s.includes('great dao') || s.includes('dao converging') || s.includes('supreme') || s.includes('dao creator') || s.includes('saint sovereign') || s.includes('eternal dao') || s.includes('dao ancestor') || s.includes('absolute existence') || s.includes('primordial')) {
    ...
    if (s.includes('primordial') || s.includes('dao ancestor')) return 'dao_ancestor_peak';
    if (s.includes('eternal dao')) return 'eternal_dao_emperor_peak';
    if (s.includes('saint')) return 'saint_peak';
...
MAJOR_SECTS.forEach(s => {
  if (!s.leaderRealm && s.leaderCultivation) {
    ...
    if (mapped) s.leaderRealm = mapped;
  }
});
...
(() => {
  const byId = new Map<string, Sect>();
  for (const s of MAJOR_SECTS) {
  ...
  MAJOR_SECTS.length = 0;
  MAJOR_SECTS.push(...Array.from(byId.values()));
})();
...
(() => {
  ...
  try {
    ...
    logger.warn('Sect leader age normalization failed:', e);
  }
})();

export const MAJOR_FACTIONS: Faction[] = [
  {
    id: 'ten_great_martial_sects_alliance',
    name: 'Ten Great Martial Sects Alliance',
    description:
      'A formal alliance of the ten great martial sects formed to coordinate defense and share restricted techniques.',
    members: MAJOR_SECTS.map(s => s.id)
  }
  ,
...
export const MAJOR_ALLIANCES: Alliance[] = [
  {
    id: 'ten_great_martial_sects_alliance',
    name: 'Ten Great Martial Sects Alliance',
    members: MAJOR_SECTS.map(s => s.id)
...
export class SectFactionSystem {
  private playerSect: string | null = null;
  private sectReputations: Record<string, number> = {};
  private factionStandings: Record<string, number> = {};
  private rivalSystem?: RivalSystem;

  constructor(rivalSystem?: RivalSystem) {
    // RivalSystem is optional for tests that instantiate SectFactionSystem without one
    if (rivalSystem) this.rivalSystem = rivalSystem as RivalSystem;
  ...
  public runTournamentForSect(sectId: string, count = 32, playerLevel = 10): { winnerId: any; result: any } | null {
    try {
  ...
  public runFactionWar(attackerFactionId: string, defenderFactionId: string, attackerStrength: number, defenderStrength: number): any {
    try {
  const ms = new MiniGameSystem();
  ...
  public joinSect(sectId: string, playerState: any): boolean {
    const sect = MAJOR_SECTS.find(s => s.id === sectId);
  ...
  public leaveSect(): void {
    if (!this.playerSect) return;
  ...
  private applySectBenefits(sect: Sect, playerState: any): void {
    if (!sect.benefits) return;
    if (Array.isArray(sect.benefits.techniques)) {
      playerState.techniques = playerState.techniques || [];
      sect.benefits.techniques.forEach((t: string) => {
        if (!playerState.techniques.includes(t)) playerState.techniques.push(t);
      });
    }
  ...
  public getAvailableSects(playerState: any): Sect[] {
    // basic requirement filter
    return MAJOR_SECTS.filter(sect => {
      const req = sect.requirements || {};
      if (req.minRealm && playerState.realm < req.minRealm) return false;
      if (req.minCombatPower && playerState.combatPower < req.minCombatPower) return false;
      return true;
    });
  }

  public getPlayerSect(): string | null {
    return this.playerSect;
  }

  public getSectReputation(sectId: string): number {
    return this.sectReputations[sectId] || 0;
  }

  public generateRivalFromSect(sectId: string, playerLevel: number): any {
    const sect = MAJOR_SECTS.find(s => s.id === sectId);
    ...
    const rivalState = this.rivalSystem && typeof (this.rivalSystem as any).getState === 'function' ? (this.rivalSystem as any).getState() : undefined;
    const rngForLevel = getRng(rivalState);
    const levelOffset = randInt(5, { rng: rngForLevel }); // 0..4
  ...
  public adjustSectReputation(sectId: string, amount: number): void {
  ...
  public getAllReputations(): Record<string, number> {
    return { ...this.sectReputations };
  }
}

export default SectFactionSystem;
