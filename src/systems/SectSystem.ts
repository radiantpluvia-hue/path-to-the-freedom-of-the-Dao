...
export interface Sect {
  ...
  name: string;
  description: string;
  type: 'righteous' | 'demonic' | 'neutral' | 'ancient' | 'hidden';
  realm: number; // Which realm they're based in
  power: number; // Overall sect strength
  reputation: number; // Player's standing with this sect
  requirements: {
    minRealm?: number;
  ...
  rivals: string[]; // IDs of rival sects
  allies: string[]; // IDs of allied sects
}
...
export interface Faction {
  ...
  name: string;
  description: string;
  type: 'political' | 'merchant' | 'military' | 'scholarly' | 'criminal';
  influence: number; // How much power they wield
  standing: number; // Player's reputation with faction
  territories: string[]; // Areas they control
  services: FactionService[];
  conflicts: string[]; // Factions they're at war with
}
...
export class SectFactionSystem {
  ...
  public joinSect(sectId: string, playerState: any): boolean {
    ...
    try {
      ...
      sect.rivals.forEach(rivalSectId => {
        ...
        const rivalSect = MAJOR_SECTS.find(s => s.id === rivalSectId);
        if (rivalSect) {
      ...
      sect.allies.forEach(allySectId => {
        ...
        const alliedSect = MAJOR_SECTS.find(s => s.id === allySectId);
        if (alliedSect) {
  ...
  public leaveSect(playerState?: any): void {
    if (!this.playerSect) {
      return;
    }

    try {
      const leavingSect = MAJOR_SECTS.find(s => s.id === this.playerSect);
  ...
  // Enhanced requirement checking with detailed feedback
  private checkSectRequirements(sect: Sect, playerState: any): { canJoin: boolean; reason?: string } {
  ...
  private calculateLeavingPenaltyMultiplier(sect: Sect | undefined, currentReputation: number): number {
    if (!sect) return 1.0;
  ...
  private adjustRivalRelationshipsForSectJoin(sect: Sect, previousSect: string | null): void {
    if (!this.rivalSystem) return;
  ...
  private adjustRivalRelationshipsForSectLeave(sect: Sect): void {
    if (!this.rivalSystem) return;
  ...
  private applyPersonalityModifierToRelationshipChange(rival: any, baseChange: number): number {
  ...
  private applySectBenefits(sect: Sect, playerState: any): void {
    if (sect.benefits.techniques) {
      sect.benefits.techniques.forEach(technique => {
        ...
        if (!playerState.techniques) playerState.techniques = [];
  ...
  private calculateRivalSectReputationLoss(joiningSect: Sect, rivalSectId: string): number {
    const baseReputationLoss = 30;
    
    // More severe penalties for certain sect type combinations
    const rivalSect = MAJOR_SECTS.find(s => s.id === rivalSectId);
    if (!rivalSect) return baseReputationLoss;
  ...
  private calculateAlliedSectReputationGain(joiningSect: Sect, alliedSectId: string): number {
    const baseReputationGain = 15;
    
    const alliedSect = MAJOR_SECTS.find(s => s.id === alliedSectId);
    if (!alliedSect) return baseReputationGain;
  ...
  private applyFactionStandingForSectRivalry(rivalSect: Sect, standingChange: number): void {
  ...
  private applyFactionStandingForSectAlliance(alliedSect: Sect, standingChange: number): void {
    // This would need faction mapping data - for now, apply to related factions
  ...
  private applyCrossSystemEffectsForSectJoin(sect: Sect, playerState: any): void {
    try {
  ...
  private applyCrossSystemEffectsForSectLeave(sect: Sect, playerState?: any): void {
    try {
  ...
  public validateSystemIntegrity(): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
  ...
  public getAvailableFactionServices(factionId: string, playerState: any): FactionService[] {
    const faction = MAJOR_FACTIONS.find(f => f.id === factionId);
    if (!faction) return [];

    const standing = this.factionStandings[factionId] || 0;
    
    return faction.services.filter(service => {
      if (service.requirements.minStanding && standing < service.requirements.minStanding) return false;
      if (service.requirements.minRealm && playerState.realm < service.requirements.minRealm) return false;
      return true;
    });
  }

  public getCurrentSect(): string | null {
    return this.playerSect;
  }
  ...
  public getFactionStanding(factionId: string): number {
  ...
  public getAllStandings(): Record<string, number> {
    return { ...this.factionStandings };
  }

  // Rival System Integration Methods
  public adjustRivalRelationship(rivalId: string, change: number): void {
  ...
  public getFactionEnemies(factionId: string): string[] {
    const faction = MAJOR_FACTIONS.find(f => f.id === factionId);
    return faction?.conflicts || [];
  }

  public getSectRivals(sectId: string): string[] {
    const sect = MAJOR_SECTS.find(s => s.id === sectId);
    return sect?.rivals || [];
  }

  public getSectAllies(sectId: string): string[] {
    const sect = MAJOR_SECTS.find(s => s.id === sectId);
    return sect?.allies || [];
  }

  public canChallengeRival(playerState: any): boolean {
    // Check if player meets requirements to challenge a rival
    const playerSect = this.playerSect;
    
    // Basic checks
    if (!playerSect) return false;
    if (playerState.combatPower < 500) return false; // Minimum combat power
  ...
  const rng = (this.rivalSystem && typeof this.rivalSystem.getRng === 'function') ? this.rivalSystem.getRng() : ((): (() => number) => {
