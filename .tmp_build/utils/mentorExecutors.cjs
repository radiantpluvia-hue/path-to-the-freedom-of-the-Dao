...
exports.mentorExecutors = {
    mentor_hidden_manual_search: (state, args) => {
    ...
    mentor_tribulation_trial: (state, args) => {
        const mentorId = args?.mentorId || 'unknown';
        // Assumes state.player.realmId is the current realm's ID
        const currentRealm = (0, gameData_1.getRealmById)(state.player.realmId || 0);
    ...
    mentor_supervised_duel: (state, args) => {
        const mentorId = args?.mentorId || 'unknown';
    ...
    mentor_sabotage_investigation: (state, args) => {
        const mentorId = args?.mentorId || 'unknown';
        ...
        if (mentorId === 'mentor_shadow_sovereign') {
            ...
            state.player.factionReputations = state.player.factionReputations || {};
            state.player.factionReputations.rivals = (state.player.factionReputations.rivals || 0) - factionDamage;
        ...
        if (mentorId === 'mentor_serene_melody') {
            ...
            state.player.factionReputations = state.player.factionReputations || {};
            state.player.factionReputations.righteous_sects = (state.player.factionReputations.righteous_sects || 0) + factionBonus;
            return { success: true, factionBonus };
        }
        return { success: roll < 0.6 };
    },
    mentor_legacy_trial: (state, args) => {
    ...
    mentor_ritual_guidance: (state, args) => {
