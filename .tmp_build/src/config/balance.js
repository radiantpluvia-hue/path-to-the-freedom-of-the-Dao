...
try {
    ...
    if (store && store.getState) {
        ...
        const p = s && s.player && s.player.settings && s.player.settings.powerScalePercent;
        if (typeof p === 'number') {
            powerScaleDecimal = Math.max(1, Math.min(100, Math.round(p))) / 100;
        }
    }
}
catch (e) { /* ignore runtime config parse errors */
    console.debug && console.debug('balance config parse failed', e);
}
    ...
    try {
        ...
        if (store && store.setState && store.getState) {
            ...
            if (typeof s.setSettings === 'function') {
                ...
                const settings = (cur.player && cur.player.settings) || {};
