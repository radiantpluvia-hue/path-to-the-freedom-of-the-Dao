...
exports.act2EventExecutors = void 0;
...
const fn_act2_first_challenge = (state) => {
    const s = { ...state, player: { ...state.player, stats: { ...state.player.stats }, factionStanding: { ...(state.player.factionStanding || {}) } }, world: { ...state.world, flags: { ...(state.world.flags || {}) } } };
    ...
    if (!s.__meta.eventsRan.includes('fn_act2_first_challenge'))
        s.__meta.eventsRan.push('fn_act2_first_challenge');
    return s;
};
...
const fn_act2_mentor_guidance = (state, args) => {
    const s = { ...state, player: { ...state.player, manuals: Array.isArray(state.player.manuals) ? [...state.player.manuals] : [], skillPoints: state.player.skillPoints || 0, stats: { ...state.player.stats } }, world: { ...state.world, flags: { ...(state.world.flags || {}) } } };
    ...
    (s.__meta || (s.__meta = {})).eventsRan = s.__meta.eventsRan || [];
    if (!s.__meta.eventsRan.includes('fn_act2_mentor_guidance'))
        s.__meta.eventsRan.push('fn_act2_mentor_guidance');
    return s;
};
...
const fn_act2_spirit_stone_mine = (state) => {
    const s = { ...state, player: { ...state.player, inventory: Array.isArray(state.player.inventory) ? [...state.player.inventory] : [], stats: { ...state.player.stats } }, world: { ...state.world, flags: { ...(state.world.flags || {}) } } };
    ...
    (s.__meta || (s.__meta = {})).eventsRan = s.__meta.eventsRan || [];
    if (!s.__meta.eventsRan.includes('fn_act2_spirit_stone_mine'))
        s.__meta.eventsRan.push('fn_act2_spirit_stone_mine');
    return s;
};
