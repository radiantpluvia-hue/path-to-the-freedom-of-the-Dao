...
function testBloodlineStatIncreases() {
    ...
    const testBloodlines = [
        bloodlines_fixed_1.ALL_BLOODLINES.find(b => b.rarity === 'common'),
        bloodlines_fixed_1.ALL_BLOODLINES.find(b => b.rarity === 'rare'),
        bloodlines_fixed_1.ALL_BLOODLINES.find(b => b.rarity === 'legendary'),
        bloodlines_fixed_1.ALL_BLOODLINES.find(b => b.rarity === 'mythical'),
        bloodlines_fixed_1.ALL_BLOODLINES.find(b => b.rarity === 'transcendent')
    ].filter(Boolean);
    testBloodlines.forEach(bloodline => {
        ...
        testRealms.forEach(realm => {
            const scaledStats = (0, scalingSystem_1.scaleBloodlineStats)(bloodline.effects.stats, realm);
            const multiplier = (0, scalingSystem_1.getRealmMultiplier)(realm);
            console.log(`  ${realm}:`);
            ...
            const expectedQi = Math.round(bloodline.effects.stats.qi * multiplier);
...
function testPhysiqueStatIncreases() {
    ...
    const testPhysiques = [
        physiques_1.PHYSIQUES.find(p => p.rarity === 'common'),
        physiques_1.PHYSIQUES.find(p => p.rarity === 'rare'),
        physiques_1.PHYSIQUES.find(p => p.rarity === 'legendary'),
        physiques_1.PHYSIQUES.find(p => p.rarity === 'mythical'),
        physiques_1.PHYSIQUES.find(p => p.rarity === 'transcendent')
    ].filter(Boolean);
    testPhysiques.forEach(physique => {
        if (!physique || !physique.effects.stats || !physique.effects.cultivation_speed)
            return;
        ...
        testRealms.forEach(realm => {
            const scaledStats = (0, scalingSystem_1.scalePhysiqueStats)(physique.effects.stats, realm);
            const scaledSpeed = physique.effects.cultivation_speed * (0, scalingSystem_1.getRealmMultiplier)(realm);
            const multiplier = (0, scalingSystem_1.getRealmMultiplier)(realm);
            console.log(`  ${realm}:`);
            console.log(`    Multiplier: ${multiplier.toFixed(2)}x`);
            console.log(`    Scaled Stats: ${JSON.stringify(scaledStats)}`);
            ...
            // Verify scaling is applied correctly
            const expectedQi = Math.round(physique.effects.stats.qi * multiplier);
            if (scaledStats.qi !== expectedQi) {
                console.log(`    ❌ QI scaling incorrect: expected ${expectedQi}, got ${scaledStats.qi}`);
            }
            else {
...
function testManualEffectIncreases() {
    ...
    const testManuals = [
        manuals_1.MANUALS.find(m => m.rank === 'common'),
        manuals_1.MANUALS.find(m => m.rank === 'rare'),
        manuals_1.MANUALS.find(m => m.rank === 'legendary'),
        manuals_1.MANUALS.find(m => m.rank === 'mythical'),
        manuals_1.MANUALS.find(m => m.rank === 'transcendent')
    ].filter(Boolean);
    testManuals.forEach(manual => {
        ...
        testRealms.forEach(realm => {
            const scaledEffects = (0, scalingSystem_1.scaleManualEffects)(manual.effects, realm);
            const multiplier = (0, scalingSystem_1.getRealmMultiplier)(realm);
            console.log(`  ${realm}:`);
            console.log(`    Multiplier: ${multiplier.toFixed(2)}x`);
...
function testRarityMultipliers() {
    ...
    rarities.forEach(rarity => {
        const multiplier = (0, scalingSystem_1.getRarityMultiplier)(rarity);
...
function testStatBalanceAcrossRarities() {
    ...
    console.log('\nBloodline Balance:');
    rarities.forEach(rarity => {
        const bloodlines = bloodlines_fixed_1.ALL_BLOODLINES.filter(b => b.rarity === rarity && b.effects.stats);
        if (bloodlines.length > 0) {
            const avgQi = bloodlines.reduce((sum, b) => sum + b.effects.stats.qi, 0) / bloodlines.length;
            const avgAtk = bloodlines.reduce((sum, b) => sum + b.effects.stats.atk, 0) / bloodlines.length;
    ...
    console.log('\nPhysique Balance:');
    rarities.forEach(rarity => {
        const physiques = physiques_1.PHYSIQUES.filter(p => p.rarity === rarity && p.effects.stats && p.effects.cultivation_speed);
        if (physiques.length > 0) {
            const avgQi = physiques.reduce((sum, p) => sum + p.effects.stats.qi, 0) / physiques.length;
            const avgSpeed = physiques.reduce((sum, p) => sum + p.effects.cultivation_speed, 0) / physiques.length;
    ...
    console.log('\nManual Balance:');
    rarities.forEach(rarity => {
        const manuals = manuals_1.MANUALS.filter(m => m.rank === rarity);
        if (manuals.length > 0) {
...
function testEdgeCases() {
    ...
    try {
        const invalidMultiplier = (0, scalingSystem_1.getRealmMultiplier)('invalid_realm');
    ...
    try {
        const invalidRarityMultiplier = (0, scalingSystem_1.getRarityMultiplier)('invalid_rarity');
    ...
    const scaledZeroStats = (0, scalingSystem_1.scaleBloodlineStats)(zeroStats, 'core_formation');
