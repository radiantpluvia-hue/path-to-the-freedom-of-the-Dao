...
const systems_3 = require("@/systems");
...
const systems_4 = require("@/systems");
...
const sharedRivalSystem = new systems_2.RivalSystem();
const sharedSectFactionSystem = new systems_2.SectFactionSystem(sharedRivalSystem);
const sharedMarketSystem = new systems_2.MarketSystem();
const sharedMentorTeachingSystem = new systems_3.MentorTeachingSystem();
const sharedMiniGameSystem = new systems_1.MiniGameSystem();
const sharedMissionSystem = new systems_3.MissionSystem();
const sharedStorySystem = new systems_3.StorySystem();
const sharedBreakthroughSystem = new systems_3.BreakthroughSystem();
const sharedEnhancedQuestSystem = new systems_3.EnhancedQuestSystem();
const sharedCraftingSystem = new systems_3.CraftingSystem(craftingRecipes_1.allRecipes);
const sharedBuffSystem = new systems_3.BuffSystem();
const systems_5 = require("@/systems");
const sharedSeclusionPath = new systems_5.SeclusionPath();
...
const normalizeRivalId = (id) => id?.replace(/^rival_/, '') || id;
const ACT1_EVENTS = [
    {
...
exports.useGameStore = (0, zustand_1.create)((set, get) => ({
    ...
    setSeclusionYears: (years) => set(state => ({
        world: {
            ...state.world,
    ...
    enterSeclusion: (years = 1, safeMode = false) => set(state => {
        try {
            get().seclusionPath.enterSeclusion(years, safeMode);
    ...
    exitSeclusion: () => set(state => {
        try {
            get().seclusionPath.exitSeclusion();
    ...
    seclusionTick: () => set(state => {
        try {
            ...
            const effects = seclusion.tick({ player: state.player, world: state.world, story: state.story, ui: state.ui, systems: state.systems });
                ...
                effects.events.forEach((e) => {
                    if (e.type === 'rare_source_found') {
                        ...
                        try {
                            ...
                            const candidate = allManuals.find((m) => m.rank === 'common') || allManuals[0];
                            if (candidate) {
                                const manualItem = {
                            ...
                            const frag = { id: 'manual_fragment', name: 'Manual Fragment', description: 'A fragment', rank: 'common', effects: {} };
                        ...
                        try {
                            ...
                            combatToSet = new systems_2.CombatSystem(playerPart, [rivalTemplate], store, store.rivalSystem, { type: 'normal' });
    ...
    performSeclusionStudy: (intensity = 1) => set(state => {
        try {
            const seclusion = get().seclusionPath;
            const res = seclusion.performStudy({ player: state.player, world: state.world, story: state.story, ui: state.ui, systems: state.systems }, intensity);
            ...
            return { player: newPlayer, world: { ...state.world, tick: state.world.tick + 1 } };
        }
        catch (e) {
    ...
    setSettings: (updates) => {
        set(state => ({ player: { ...state.player, settings: { ...(state.player.settings || {}), ...updates } } }));
    },
    // Mini-game API
    ...
    getVisibleStats: () => {
        ...
        return {
            ...
            atk: (0, statTiers_1.tierLabelFor)(p.stats?.atk ?? 0),
            def: (0, statTiers_1.tierLabelFor)(p.stats?.def ?? 0),
            speed: (0, statTiers_1.tierLabelFor)(p.stats?.speed ?? 0)
        };
    },
    // Debug
    attachToWindow: () => {
    ...
    attemptMission: (missionId) => {
        ...
        if (result.success && result.rewards) {
            ...
            const scaledRewards = playtestScaling_1.PlaytestScaling.applyScaledEffects(result.rewards, { source: 'quest' });
            const updates = {};
    ...
    requestSectMission: () => {
        ...
        const hasActiveSectMission = (store.story.quests || []).some((q) => q.id?.startsWith('sect_mission_') && q.status === 'active');
        if (hasActiveSectMission) {
            store.addEventLog('You already have an active sect mission. Complete it first.');
            return;
        }
        const newMission = (0, systems_4.generateSectMission)(store.player);
        set(state => ({
            story: {
                ...state.story,
    ...
    seekRefuge: () => {
        ...
        set(state => ({ ui: { ...state.ui, activeStoryChoice: refugeEvent } }));
    },
    ...
    experimentWithIngredients: (ingredientItemIds) => {
        ...
        if (result.expGained > 0)
            store.gainSkillExp('alchemy', result.expGained); // Assuming alchemy for now
    },
    craftItem: (recipeId) => {
        const store = get();
        ...
        // 3. Grant skill EXP
        if (result.expGained > 0)
            store.gainSkillExp(String(recipe.skill), result.expGained);
    },
    ...
    useItem: (inventoryIndex) => {
        ...
        if (effects.permanent_stat_boost) {
            ...
            for (const [stat, value] of Object.entries(effects.permanent_stat_boost)) {
                if (typeof value === 'number') {
                    // Apply to both base (for permanence) and current (for immediate effect)
                    playerUpdate.baseStats[stat] = (playerUpdate.baseStats[stat] || 0) + value;
                    playerUpdate.stats[stat] = (playerUpdate.stats[stat] || 0) + value;
                ...
                if (buff.duration <= 0) {
                    ...
                    setTimeout(() => {
                        try {
                            const s = get();
                            let p = { ...s.player };
                            const idx = p.activeBuffs.findIndex((b) => b.id === idToRemove);
                            if (idx === -1)
                                return;
    ...
    resetDailyQuests: () => {
        ...
        set(state => ({ player: systems_3.DailyLoopSystem.resetDailyForPlayer({ ...state.player }) }));
    ...
    checkDailyReset: () => {
        ...
        if (systems_3.DailyLoopSystem.hasDayPassed(lastReset)) {
            ...
            try {
                const currentDay = store.world?.day || Math.floor(Date.now() / systems_3.DailyLoopSystem.ONE_DAY_MS);
    ...
    tickDaily: () => {
        ...
        try {
            const newDay = get().world.day || Math.floor(Date.now() / systems_3.DailyLoopSystem.ONE_DAY_MS);
    ...
    learnManualById: (manualId) => {
        try {
            ...
            const allManuals = (manualModule?.ALL_MANUALS || manualModule?.MANUALS || manualModule?.manuals) || [];
            ...
            if (effects.skills) {
                Object.entries(effects.skills).forEach(([skillId, bonus]) => {
    ...
    attemptStudy: (manualId) => {
        try {
            ...
            const derivedQiFromEffects = Math.floor((((manual.effects && manual.effects.qiGathering) || 0) / 50));
    ...
    canEvolveManual: (manualId) => {
        const state = get();
        const owned = state.player.manuals.find(m => m.id === manualId);
        if (!owned)
            return { can: false, unmet: ['manual_not_owned'] };
        const req = owned.evolutionRequirements || {};
        const unmet = [];
        // Supported requirement keys (extensible): minRealm, minQi, skills, stats, hasManual, special
        if (req.minRealm && state.player.realmId !== req.minRealm)
            unmet.push('minRealm');
        if (typeof req.minQi === 'number' && (state.player.currentQi || 0) < req.minQi)
            unmet.push('minQi');
        if (req.skills && typeof req.skills === 'object') {
            Object.entries(req.skills).forEach(([skillId, lvl]) => {
                if (!state.player.skills[skillId] || state.player.skills[skillId].level < lvl)
                    unmet.push(`skill:${skillId}`);
            });
        }
        if (req.stats && typeof req.stats === 'object') {
            Object.entries(req.stats).forEach(([statId, val]) => {
                const v = state.player.stats[statId] || 0;
                if (v < val)
                    unmet.push(`stat:${statId}`);
            });
        }
        if (req.hasManual) {
            const has = state.player.manuals.some(m => m.id === req.hasManual);
            if (!has)
                unmet.push(`hasManual:${req.hasManual}`);
        }
    ...
    awakenBloodlineIfPossible: () => {
        ...
        if (!bl || !bl.awakening_requirements || !bl.awakenedId)
            return false;
        ...
        const awakened = data_1.ALL_BLOODLINES.find((b) => b.id === bl.awakenedId);
        if (!awakened)
            return false;
        // Apply awakened effects: merge stats/skills and replace bloodline
        const mergedStats = { ...state.player.stats };
        const applyStats = (effects) => {
            if (!effects?.stats)
                return;
            const hasBase = (v) => typeof v === 'object' && v !== null && typeof v.base === 'number';
            Object.entries(effects.stats).forEach(([key, val]) => {
                if (typeof val === 'number') {
        ...
        const mergedSkills = { ...state.player.skills };
        if (awakened.effects?.skills) {
            Object.entries(awakened.effects.skills).forEach(([skillId, bonus]) => {
                const num = typeof bonus === 'number' ? bonus : Number(bonus) || 0;
                if (mergedSkills[skillId])
                    mergedSkills[skillId] = { ...mergedSkills[skillId], level: Math.max(mergedSkills[skillId].level, num) };
            });
        }
        set({
            player: {
        ...
        return set(state => {
            ...
            while (newSkill.exp >= newSkill.expToNext) {
                ...
                if ([5, 10, 15, 20, 25, 30].includes(newSkill.level)) {
                    const evolutionOptions = skillEvolutionPaths[skillId];
    ...
    evolveSkill: (baseSkillId, evolutionPath) => set(state => {
        const skillEvolutionPaths = {
            weaponMastery: ['Sword Qi', 'Spear Arts', 'Bow Arts', 'Dagger Arts', 'Staff Arts'],
            swordsmanship: ['Sword Qi', 'Sword Domain'],
            spearArts: ['Spear Intent', 'Dragon Spear Arts'],
        ...
        if (!baseSkill)
            return state;
        const evolutionOptions = skillEvolutionPaths[baseSkillId];
    ...
    checkMinorStageBreakthrough: () => set(state => {
        const currentRealm = cultivationRealms_1.REALM_ORDER[state.player.realmId - 1];
        let currentStage = state.player.minorStage;
        const maxStages = cultivationRealms_1.CULTIVATION_REALMS[currentRealm]?.minorStages || 1;
    ...
    checkRealmBreakthrough: () => set(state => {
        ...
        if (currentStage === maxStages && state.player.currentQi >= state.player.qiRequired) {
            const nextRealm = cultivationRealms_1.CULTIVATION_REALMS[currentRealm]?.nextRealm;
            if (nextRealm) {
                const breakthroughDifficulty = cultivationRealms_1.CULTIVATION_REALMS[currentRealm]?.breakthroughDifficulty || 1;
                const lifespanBonus = cultivationRealms_1.CULTIVATION_REALMS[nextRealm]?.lifespanBonus || 0;
                ...
                return {
                    player: {
                        ...
                        qiRequired: cultivationRealms_1.REALM_QI_REQUIREMENTS[nextRealm],
    ...
    addToInventory: (item) => set(state => {
        ...
        const grantWeaponExp = () => {
            ...
            if (name.includes('jian') || name.includes('sword'))
                get().gainSkillExp('swordsmanship', 1);
            else if (name.includes('spear'))
                get().gainSkillExp('spearArts', 1);
            else if (name.includes('bow'))
                get().gainSkillExp('archery', 1);
            else if (name.includes('dagger') || name.includes('blade'))
                get().gainSkillExp('daggerArts', 1);
            else if (name.includes('staff'))
                get().gainSkillExp('staffArts', 1);
        ...
        const applyManualFromItem = () => {
            if (!item?.type || item.type !== 'manual' || !item.id)
                return null;
            // eslint-disable-next-line @typescript-eslint/no-var-requires
            const allManuals = (require('@/data/manuals')?.manuals) || [];
    ...
    triggerRandomEvent: () => {
        if ((0, seededRng_1.runtimeRng)() < 0.3) {
    ...
    cultivate: () => set(state => {
        ...
        const currentRealmKey = state.player.realmId ? cultivationRealms_1.REALM_ORDER[state.player.realmId - 1] : 'mortal';
        const currentRealmIndex = cultivationRealms_1.REALM_ORDER.indexOf(currentRealmKey);
        const goldenImmortalIndex = cultivationRealms_1.REALM_ORDER.indexOf('golden_immortal');
        const years = currentRealmIndex >= goldenImmortalIndex
            ? Math.max(1, Number(state.world.flags?.seclusionYears) || 1)
            : 1;
        ...
        const session = (0, cultivationMechanics_1.generateCultivationSession)(currentRealmKey, state.player.talentId || 'average', minutes, [] // Modifiers (env/state/bloodline/physique/manual) can be integrated later
        );
    ...
    explore: () => set(state => {
        ...
        if ((0, seededRng_1.runtimeRng)() < 0.3) {
            ...
            const foundItem = items[Math.floor((0, seededRng_1.runtimeRng)() * items.length)];
    ...
    saveGame: () => {
        try {
            const state = get();
            const success = systems_2.SaveLoadSystem.saveGame({
                player: state.player,
                world: state.world,
                story: state.story,
                ui: state.ui,
    ...
    loadGame: () => {
        try {
            const saveData = systems_2.SaveLoadSystem.loadGame();
    ...
    finalizeCharacterCreation: (payload) => {
        const store = get();
        const safePayload = payload || { name: store.player.name || 'Player', gender: store.player.gender || 'Male', race: store.player.race || 'Human', background: store.player.background || null };
        const { name, gender, race, background } = safePayload;
        // Basic validation
        if (!name || !race || !gender) {
    ...
    nextLore: () => set(state => {
        ...
        if (nextIndex >= taiYungLore_1.TAI_YUNG_OPENING_LORE.length) {
            return {
    ...
    assignRandomBloodline: () => set(state => {
        const availableBloodlines = data_1.ALL_BLOODLINES.filter((b) => b.rarity === 'common' ||
            (b.rarity === 'uncommon' && (0, seededRng_1.runtimeRng)() < 0.3) ||
            (b.rarity === 'rare' && (0, seededRng_1.runtimeRng)() < 0.1));
    ...
    assignRandomPhysique: () => set(state => {
        const availablePhysiques = physiques_1.PHYSIQUES.filter(p => p.rarity === 'common' ||
            (p.rarity === 'uncommon' && (0, seededRng_1.runtimeRng)() < 0.2) ||
            (p.rarity === 'rare' && (0, seededRng_1.runtimeRng)() < 0.05));
    ...
    startCombatWithRival: (rivalId) => {
        ...
        try {
            const ai = store.rivalSystem?.getAISystem ? store.rivalSystem.getAISystem() : null;
            if (ai && typeof ai.generateAdaptiveDialogue === 'function') {
                const line = ai.generateAdaptiveDialogue(rival, 'combat_start');
                if (line)
                    store.addEventLog(`${rival.name}: ${line}`);
            }
        }
        catch (e) {
            /* intentionally ignored */
    ...
    startRivalEncounter: (rivalId) => {
        const store = get();
        const status = (0, rivalSystemEnhancements_1.canEncounterRivalEnhanced)(store.rivalSystem, rivalId, store.world.day, store.player.realmId || 1);
    ...
    adjustRivalRelationship: (rivalId, change) => set(state => {
        const store = get();
        ...
        const result = (0, rivalSystemEnhancements_1.adjustRivalRelationshipEnhanced)(store.rivalSystem, gameState, rivalId, change, currentDay);
        if (!result.success) {
    ...
    markRivalDefeated: (rivalId) => set(state => {
    ...
    canEncounterRival: (rivalId) => {
        const store = get();
        try {
            const result = (0, rivalSystemEnhancements_1.canEncounterRivalEnhanced)(store.rivalSystem, rivalId, store.world.day, store.player.realmId || 1);
    ...
    getRivalEncounterStatus: (rivalId) => {
        const store = get();
        try {
            return (0, rivalSystemEnhancements_1.canEncounterRivalEnhanced)(store.rivalSystem, rivalId, store.world.day, store.player.realmId || 1);
        }
        catch (error) {
    ...
    recordRivalEncounter: (rivalId) => set(state => {
        const store = get();
        try {
    ...
    startFactionBattle: (factionId, enemyFactionId) => {
        const store = get();
    ...
    resolveFactionBattle: (battleId, outcome) => set(state => {
        const store = get();
        try {
            // Use enhanced resolver to compute cascading effects and ensure sync
            const result = (0, rivalSystemEnhancements_1.resolveFactionBattleEnhanced)(store.rivalSystem, store.sectFactionSystem, battleId, outcome, {
                player: state.player,
                world: state.world,
                story: state.story,
                ui: state.ui,
            ...
            const scaledEffects = result.effects.map(e => playtestScaling_1.PlaytestScaling.applyScaledEffects(e, { source: 'faction_battle' }));
                ...
                try {
                    switch (effect.type) {
                        ...
                        case 'sect_reputation':
                            if (effect.sect)
                                store.adjustSectReputation(effect.sect, effect.change);
                            break;
                        case 'rival_relationship':
                            if (effect.rivalId)
                                store.adjustRivalRelationship(effect.rivalId, effect.change);
                            break;
                        case 'world_event':
    ...
    adjustSectReputation: (sectId, amount) => set(state => {
    ...
    adjustFactionStanding: (factionId, amount) => set(state => {
    ...
    canAccessFactionService: (factionId, serviceId) => {
        ...
        const hostileRivals = rivals.filter((r) => r.faction === factionId && r.relationship < -40);
    ...
    purchaseFactionService: (factionId, serviceId) => {
        const store = get();
        ...
        set(state => {
            ...
            if (service.effects) {
                if (service.effects.qi) {
                    newState.player.currentQi = Math.min(newState.player.currentQi + service.effects.qi, newState.player.qiRequired);
                }
    ...
    triggerFactionBattleEvent: (playerFaction, enemyFaction) => {
        const store = get();
    ...
    attemptMinorBreakthrough: () => {
        const store = get();
        if (!store.player.realmId)
            return false;
        const realmKey = cultivationRealms_1.REALM_ORDER[store.player.realmId - 1] || 'mortal';
        const result = store.breakthroughSystem.attemptMinorBreakthrough(realmKey, store.player.minorStage || 1, {
    ...
    attemptRealmBreakthrough: (challengeId) => {
        const store = get();
        if (!store.player.realmId)
            return false;
        const currentRealmKey = cultivationRealms_1.REALM_ORDER[store.player.realmId - 1];
        const result = store.breakthroughSystem.attemptRealmBreakthrough(currentRealmKey, challengeId, {
            daoHeart: store.player.daoHeart || 0,
            stability: store.player.stability || 0,
            karma: store.player.karma || 0,
    ...
    getBreakthroughChallenges: (realm) => {
        const store = get();
    ...
    canAttemptMinorBreakthrough: () => {
        const store = get();
        if (!store.player.realmId)
            return { canAttempt: false, reason: 'No realm set' };
        const currentRealmKey = cultivationRealms_1.REALM_ORDER[store.player.realmId - 1] || 'mortal';
        return store.breakthroughSystem.canAttemptMinorBreakthrough(currentRealmKey, store.player.minorStage || 1, store.player.currentQi, store.player.stability || 0);
    },
    canAttemptRealmBreakthrough: () => {
        ...
        if (!store.player.realmId)
            return { canAttempt: false, reason: 'No realm set' };
        const currentRealmKey = cultivationRealms_1.REALM_ORDER[store.player.realmId - 1] || 'mortal';
        return store.breakthroughSystem.canAttemptRealmBreakthrough(currentRealmKey, store.player.minorStage || 1, {
            daoHeart: store.player.daoHeart || 0,
            stability: store.player.stability || 0,
            karma: store.player.karma || 0,
            hp: store.player.hp || 0
    ...
    enableDemoMode: () => set(state => {
        const demoState = {
            player: {
                ...
                yuan: 1000,
                karma: 50,
                skills: Object.fromEntries(Object.entries(state.player.skills).map(([key, skill]) => [
                    key,
    ...
    maxAllSkills: () => set(state => {
        const maxedSkills = Object.fromEntries(Object.entries(state.player.skills).map(([key, skill]) => [
            key,
    ...
    gainResources: (amount) => set(state => {
    ...
    advanceRealm: () => set(state => {
        ...
        const currentRealm = cultivationRealms_1.REALM_ORDER[currentIndex];
        const nextRealm = currentIndex < cultivationRealms_1.REALM_ORDER.length - 1 ? cultivationRealms_1.REALM_ORDER[currentIndex + 1] : currentRealm;
