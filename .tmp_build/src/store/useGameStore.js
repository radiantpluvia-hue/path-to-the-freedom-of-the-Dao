...
exports.pickInitialWorldType = pickInitialWorldType;
...
const taiYungLore_1 = require("../data/taiYungLore");
...
const SectSystem_1 = __importDefault(require("../systems/SectSystem"));
...
const practiceConfig_1 = __importStar(require("@/config/practiceConfig"));
const DailyLoopSystem_1 = require("../systems/DailyLoopSystem");
const MissionSystem_1 = require("../systems/MissionSystem");
const StorySystem_1 = require("../systems/StorySystem");
const LifeArcSystem_1 = require("../systems/LifeArcSystem");
const BreakthroughSystem_1 = require("../systems/BreakthroughSystem");
const EnhancedQuestSystem_1 = require("../systems/EnhancedQuestSystem");
const CraftingSystem_1 = require("../systems/CraftingSystem");
const BuffSystem_1 = require("../systems/BuffSystem");
const MentorTeachingSystem_1 = require("../systems/MentorTeachingSystem");
const LifePhaseSystem_1 = __importDefault(require("../systems/LifePhaseSystem"));
...
const proficiency_1 = require("@/utils/proficiency");
const playtestScaling_1 = require("../utils/playtestScaling");
const SectMissionSystem_1 = require("../systems/SectMissionSystem");
const rivalSystemEnhancements_1 = require("./rivalSystemEnhancements");
...
const TrainingEngine = __importStar(require("../game/training"));
...
function toCombatTechniqueSync(skill) {
    if (__skillRegistry && typeof __skillRegistry.toCombatTechnique === 'function')
        return __skillRegistry.toCombatTechnique(skill);
    // kick off background load
    void __ensureSkillRegistry();
    return skill;
}
function getActiveAbilityByIdSync(id) {
    if (__skillRegistry && typeof __skillRegistry.getSkillById === 'function')
        return __skillRegistry.getSkillById(id);
    void __ensureSkillRegistry();
    return null;
}
const EquipmentSystem_1 = __importStar(require("@/systems/EquipmentSystem"));
...
void rng_1.choice;
void practiceConfig_1.default;
void DailyLoopSystem_1.DailyLoopSystem;
void mentorTeachingsLoader_1.mentorTeachingsLoader;
void playtestScaling_1.PlaytestScaling;
void rivalSystemEnhancements_1.adjustRivalRelationshipEnhanced;
void rivalSystemEnhancements_1.resolveFactionBattleEnhanced;
void rivalSystemEnhancements_1.canEncounterRivalEnhanced;
void rivalSystemEnhancements_1.validateRivalSystemIntegrity;
void cultivationMechanics_1.calculateCultivationSpeed;
void cultivationMechanics_1.calculateBreakthroughChance;
void cultivationMechanics_1.generateBreakthroughAttempt;
void EquipmentSystem_1.defaultEquipmentSnapshot;
// Shared system instances (singletons)
const sharedRivalSystem = new RivalSystem_1.RivalSystem();
const sharedSectFactionSystem = new SectSystem_1.default(sharedRivalSystem);
...
let _sharedMarketSystem = null;
function getSharedMarketSystem() {
    if (_sharedMarketSystem != null)
        return _sharedMarketSystem;
    ...
    const ensureLoaded = async () => {
        if (real)
            return real;
        try {
            const mod = await Promise.resolve().then(() => __importStar(require('../systems/MarketSystem')));
            if (mod && typeof mod.MarketSystem === 'function') {
                real = new mod.MarketSystem();
            }
        ...
        for (const q of queue) {
            try {
                if (q.type === 'get')
                    q.resolve && q.resolve(real ? real[q.prop] : undefined);
                else if (q.type === 'set')
                    real[q.prop] = q.value;
            }
            catch (e) { /* ignore */ }
    ...
    const proxy = new Proxy({}, {
        get(_t, prop) {
            ...
            Object.assign(SYNC_FALLBACKS, {
                getMarketSummary: () => ({ markets: [] }),
                listOpenAuctions: () => [],
                getAuctionById: (_id) => null,
                getMarketStats: () => ({ totalAuctions: 0, activeMarkets: 0 }),
                listMarketsSummary: () => ({ markets: [] })
            });
            ...
            return (...args) => {
                ...
                // ensure load started
                void ensureLoaded();
                return p.then((fn) => typeof fn === 'function' ? fn.apply(real, args) : undefined);
            };
        },
        set(_t, prop, value) {
            if (real) {
...
const sharedMarketSystem = new Proxy({}, {
    ...
    getOwnPropertyDescriptor(_target, prop) {
        const inst = getSharedMarketSystem();
        return Object.getOwnPropertyDescriptor(inst, prop) || undefined;
    }
});
const sharedMentorTeachingSystem = new MentorTeachingSystem_1.MentorTeachingSystem();
const sharedMissionSystem = new MissionSystem_1.MissionSystem();
const sharedStorySystem = new StorySystem_1.StorySystem();
const sharedLifeArcSystem = new LifeArcSystem_1.LifeArcSystem();
const sharedLifePhaseSystem = new LifePhaseSystem_1.default();
const sharedBreakthroughSystem = new BreakthroughSystem_1.BreakthroughSystem();
const sharedEnhancedQuestSystem = new EnhancedQuestSystem_1.EnhancedQuestSystem();
const sharedCraftingSystem = new CraftingSystem_1.CraftingSystem(craftingRecipes_1.allRecipes);
const sharedBuffSystem = new BuffSystem_1.BuffSystem();
const sharedSeclusionPath = new seclusionPath_1.SeclusionPath();
const sharedMiniGameSystem = new MiniGameSystem_1.MiniGameSystem();
// Narrative-related shared systems
const NarrativeEngine_1 = require("@/systems/NarrativeEngine");
const dialogues_1 = require("@/data/dialogues");
const LegacySystem_1 = require("@/systems/LegacySystem");
const HeavenlyDaoSystem_1 = require("@/systems/HeavenlyDaoSystem");
const narrativeDemo_1 = __importDefault(require("@/data/narrativeDemo"));
const monologueTriggers_1 = __importDefault(require("@/data/monologueTriggers"));
const sharedNarrativeEngine = new NarrativeEngine_1.NarrativeEngine();
const sharedLegacySystem = new LegacySystem_1.LegacySystem(sharedNarrativeEngine);
const sharedHeavenlyDaoSystem = new HeavenlyDaoSystem_1.HeavenlyDaoSystem(sharedNarrativeEngine);
...
const _pendingLoadQueue = [];
// Register a small demo trigger so the NarrativeEngine returns something out-of-the-box
try {
    sharedNarrativeEngine.registerTrigger(narrativeDemo_1.default);
}
catch (e) {
    logger_1.logger.debug('useGameStore: registering demo narrative trigger failed', e);
}
// Register monologue triggers (non-fatal if any registration fails)
try {
    if (Array.isArray(monologueTriggers_1.default)) {
        for (const t of monologueTriggers_1.default) {
            try {
                sharedNarrativeEngine.registerTrigger(t);
            }
            catch (e) {
                logger_1.logger.debug('useGameStore: register monologue trigger failed', e);
            }
        }
    }
}
catch (e) {
    logger_1.logger.debug('useGameStore: monologue triggers registration failed', e);
}
...
(async () => {
    const mod = await (0, safeImport_1.safeImport)(() => Promise.resolve().then(() => __importStar(require('@/data/encounterNarratives'))));
    try {
        if (mod && typeof mod.initEncounterNarratives === 'function')
            await mod.initEncounterNarratives();
    }
    catch (e) {
        logger_1.logger.debug('useGameStore: initEncounterNarratives failed', e);
    }
})();
const ACT1_EVENTS = [
    {
...
function pickInitialWorldType() {
    try {
        // Allow deterministic test override
        const override = globalThis.__WORLD_OVERRIDE__ || (process && process.env && process.env.WORLD_OVERRIDE);
        if (override === 'murim' || override === 'cultivation')
            return override;
    }
    catch { /* ignore */ }
    const r = (0, rng_1.getRng)()();
...
const balance_2 = require("../config/balance");
const BASE_MUL = (0, balance_2.getBaseNumberMultiplier)();
const initialGameState = {
    player: {
        ...
        resourcefulness: 50,
        currentQi: 0,
        qiRequired: cultivationRealms_1.REALM_QI_REQUIREMENTS[cultivationRealms_1.REALM_ORDER[0]],
    ...
    systems: {
        ...
        questObjectives: {},
        questRewards: {},
        domain: DomainSystem_1.DomainSystem.defaultState()
    }
};
...
exports.useGameStore = (0, zustand_1.create)((set, get) => ({
    ...
    setUIProperty: (key, value) => {
        ...
        try {
            ...
            if (key === 'dialoguePanelPosition' || key === 'dialoguePanelOpacity' || key === 'showRealmList') {
                try {
                    get()._persistUIPrefs?.();
                }
                catch (e) { /* ignore */ }
            ...
            logger_1.logger.debug('RivalAISystem attach failed', e);
        }
    },
    // Power scaling setter
    setPowerScalePercent: (pct) => {
        ...
        try {
            get().addEventLog?.(`Power scale set to ${safe}%`);
        }
        catch (e) {
            void e;
        }
    ...
    showTechniqueMastery: false,
    setShowTechniqueMastery: (v) => set(state => ({ ui: { ...state.ui, ...{ showTechniqueMastery: v } } })),
    getTechniqueMasteryProgress: () => {
        try {
            ...
            const results = ids.map((t) => {
                if (typeof t === 'string') {
                    // attempt to resolve from registry via skillRegistry helper
                    try {
                        const _skill = get().storySystem?.skillRegistry?.getSkillById ? get().storySystem.skillRegistry.getSkillById(t) : null;
                        void _skill;
                        // fallback: check active abilities registry via toCombatTechnique
                        const reg = get()._getActiveAbilityById?.(t) || null;
    ...
    practiceTechnique: (techniqueId) => {
        try {
            const store = get();
            const player = store.player;
            // use tuning constants from config
            // consts: COST_QI, ADD_FATIGUE, PRACTICE_XP, RANK_XP
            if ((player.currentQi || 0) < practiceConfig_1.COST_QI) {
                set(s => ({ eventLog: [...s.eventLog, `Not enough Qi to practice ${techniqueId}. Need ${practiceConfig_1.COST_QI}.`] }));
                return { success: false, message: 'Insufficient Qi' };
            }
            try {
                get().startMonologueByTag('training');
            }
            catch (e) {
                void e;
            }
            ...
            const newFatigue = currentFatigue + practiceConfig_1.ADD_FATIGUE;
            // Deduct Qi and apply fatigue
            set(s => ({ player: { ...s.player, currentQi: Math.max(0, (s.player.currentQi || 0) - practiceConfig_1.COST_QI), ...{ fatigue: newFatigue } } }));
            // Resolve botch using configured thresholds and chances
            let botchChance = 0;
            if (newFatigue >= practiceConfig_1.BOTCH_HIGH_FATIGUE_THRESHOLD)
                botchChance = practiceConfig_1.BOTCH_HIGH_CHANCE;
            else if (currentFatigue >= practiceConfig_1.BOTCH_LOW_FATIGUE_THRESHOLD)
                botchChance = practiceConfig_1.BOTCH_LOW_CHANCE;
            ...
            if (rollVal < botchChance) {
                ...
                set(s => ({
                    player: { ...s.player, ...{ fatigue: (s.player.fatigue || 0) + practiceConfig_1.BOTCH_EXTRA_FATIGUE } },
                ...
                try {
                    const techMeta = get().storySystem?.skillRegistry?.getSkillById?.(techniqueId) || get()._getActiveAbilityById?.(techniqueId) || null;
                    const isWeapon = techMeta?.weaponType || (typeof techniqueId === 'string' && techniqueId.includes('weapon'));
                    if (isWeapon)
                        try {
                            get().startMonologueByTag('weapon');
                        }
                        catch (e) {
                            void e;
                        }
                        ...
                        try {
                            get().startMonologueByTag('cultivation');
                        }
                        catch (e) {
                            void e;
                        }
                    try {
                        get().startMonologueByTag('botch');
                    }
                    catch (e) {
                        void e;
                    }
                ...
                try {
                    ...
                    if (buffSys?.createBuff && buffSys?.applyBuff) {
                        ...
                        set(s => ({ player: { ...s.player, activeBuffs: [...(s.player.activeBuffs || []), { id: sid, name: 'Strained', effects: { stats: { atk: -0.1 } }, duration: 1 }] } }));
                    }
                }
                catch (e) {
                    void e;
                ...
                setTimeout(() => { try {
                    set(s => ({ ui: { ...s.ui, lastPracticeMessage: undefined } }));
                }
                catch (e) {
                    void e;
                } }, practiceConfig_1.INLINE_MESSAGE_DURATION_MS);
            ...
            set(s => {
                const techniques = Array.isArray(s.player.techniques) ? [...s.player.techniques] : [];
                let updated = false;
                for (let i = 0; i < techniques.length; i++) {
                    const t = techniques[i];
                    if ((typeof t === 'string' && t === techniqueId) || (typeof t === 'object' && t && t.id === techniqueId)) {
                        // Ensure object form
                        const obj = (typeof t === 'string') ? { id: techniqueId, name: techniqueId, masteryXp: 0, masteryRank: 0 } : { ...t };
                        obj.masteryXp = (obj.masteryXp || 0) + practiceConfig_1.PRACTICE_XP;
                        // Rank-up using configured RANK_XP
                        while ((obj.masteryXp || 0) >= practiceConfig_1.RANK_XP) {
                            obj.masteryXp = (obj.masteryXp || 0) - practiceConfig_1.RANK_XP;
                ...
                if (!updated) {
                    return { eventLog: [...s.eventLog, `You tried to practice ${techniqueId} but you don't know that technique.`] };
                }
                return { player: { ...s.player, techniques }, eventLog: [...s.eventLog, `You practiced ${techniqueId} and gained mastery.`] };
                const ev = { player: { ...s.player, techniques }, eventLog: [...s.eventLog, `You practiced ${techniqueId} and gained mastery.`] };
                ...
                try {
                    ...
                    if (r2 < 0.15)
                        get().startMonologueByTag(`${seqTag}_epiphany`);
                    else
                        get().startMonologueByTag(`${seqTag}_sequence`);
                }
                catch (e) {
                    try {
                        get().startMonologueByTag('training');
                ...
                // set inline UI message
                set(s => ({ ui: { ...s.ui, lastPracticeMessage: `+${practiceConfig_1.PRACTICE_XP} XP to ${techniqueId}` } }));
                setTimeout(() => { try {
                    set(s => ({ ui: { ...s.ui, lastPracticeMessage: undefined } }));
                }
                catch (e) {
            ...
            try {
                set(s => ({ eventLog: [...s.eventLog, `Practice failed for ${techniqueId}.`] }));
            }
            catch (e) {
                void e;
            }
    ...
    setCultivationDaysAllocated: (days) => set(state => ({
        player: { ...state.player, cultivationDaysAllocated: Math.max(0, Math.min(365, days)) }
    })),
    eventLog: ['Welcome to your cultivation journey...'],
    realms: cultivationRealms_1.CULTIVATION_REALMS,
    backgrounds: [],
    events: ACT1_EVENTS,
    quests: QUESTS,
    taiYungLore: taiYungLore_1.TAI_YUNG_OPENING_LORE,
    bloodlines: data_1.ALL_BLOODLINES,
    physiques: physiques_1.PHYSIQUES,
    // Dialogue system state
    dialogueQueue: [],
    currentDialogue: null,
    currentLineIndex: -1,
    monologueIntervalMs: 60000, // default 60s between ambient monologues
    lastMonologueTick: 0,
    // per-monologue cooldown timestamps (ms since epoch)
    _monologueCooldowns: {},
    // default cooldown in ms to avoid repeats
    _defaultMonologueCooldownMs: 60000,
            ...
            const sched = () => {
                try {
                    ...
                    if ((now - (store.lastMonologueTick || 0)) >= (store.monologueIntervalMs || 60000)) {
                        ...
                        if (res.success) {
                            set(_state => ({ lastMonologueTick: now }));
                        }
                    }
                }
                catch (e) { /* ignore */ }
    ...
    playMusic: (trackId) => set(_state => ({ musicPlaying: true, currentMusicTrack: trackId })),
    stopMusic: () => set(_state => ({ musicPlaying: false, currentMusicTrack: null })),
    // Music settings
    musicVolume: 0.6,
    musicMuted: false,
    // Whether autoplay was blocked by the browser — UI can use this to prompt user interaction
    autoplayBlocked: false,
    setAutoplayBlocked: (b) => set(_state => ({ autoplayBlocked: b })),
    ...
    requestEnableAudio: () => set(() => ({ enableAudioRequestTimestamp: Date.now() })),
    setMusicVolume: (v) => {
        set(_state => ({ musicVolume: Math.max(0, Math.min(1, v)) }));
        try {
            get()._persistAudioPrefs?.();
        }
        catch (e) { /* ignore */ }
    },
    setMusicMuted: (m) => {
        set(_state => ({ musicMuted: m }));
        try {
            get()._persistAudioPrefs?.();
        }
        catch (e) { /* ignore */ }
            ...
            try {
                if (typeof localStorage !== 'undefined') {
                    ...
                    if (raw) {
                        ...
                        set(state => ({ musicVolume: typeof parsed.musicVolume === 'number' ? Math.max(0, Math.min(1, parsed.musicVolume)) : state.musicVolume, musicMuted: !!parsed.musicMuted }));
                    }
                }
            }
            catch (e) {
    ...
    _persistUIPrefs: () => {
        try {
            if (typeof localStorage !== 'undefined') {
                const s = get();
                const payload = {
                    dialoguePanelPosition: s.ui.dialoguePanelPosition,
                    dialoguePanelOpacity: s.ui.dialoguePanelOpacity,
                    showRealmList: s.ui.showRealmList
                };
    ...
    _callOnPlayerActionWithGu: (reason) => {
        try {
            // Attempt dynamic import to avoid bundler CJS requires
            (async () => {
                try {
                    const gu = await Promise.resolve().then(() => __importStar(require('../systems/guBranch')));
                    if (gu && typeof gu.onPlayerActionWithGu === 'function') {
                        try {
                            gu.onPlayerActionWithGu(get(), reason || 'action');
                        }
                        catch (e) {
                            void e;
                        }
            ...
            try {
                ...
                timer = setTimeout(() => {
                    try {
                        ...
                        try {
                            localStorage.setItem('xianxia_transcript', t);
                        }
                        catch (e) {
                            void e;
                        }
    ...
    storySystem: (() => {
        try {
            const env = globalThis.__DEFAULT_STORY_SYSTEM__ ?? (typeof process !== 'undefined' && process.env && process.env.DEFAULT_STORY_SYSTEM);
            if (env === 'lifearc')
                return sharedLifeArcSystem;
        }
        catch (e) {
            // ignore
        }
    ...
    _initAIOnce: (() => {
        ...
        return () => {
            ...
            try {
                ...
                void Promise.resolve().then(() => __importStar(require('@/systems/RivalAISystem'))).then(mod => {
                    ...
                    .catch((err) => {
                    logger_1.logger.warn('RivalAISystem module failed to load', err);
                });
            }
            catch (e) {
    ...
    setSeclusionYears: (years) => set(_state => ({
        world: {
            ..._state.world,
    ...
    enterSeclusion: (years = 1, safeMode = false) => set(_state => {
        try {
            ...
            const newUi = { ..._state.ui, isInSeclusion: true, seclusionYears: years, seclusionProgress: 0, seclusionReadyForBreakthrough: false };
            const newLog = [...(_state.eventLog || []), `You adopt a seclusion for ${years} year(s).`];
            return { player: { ..._state.player }, ui: newUi, eventLog: newLog };
        }
        catch (e) {
            logger_1.logger.error('enterSeclusion failed', e);
            return _state;
        }
    }),
    exitSeclusion: () => set(_state => {
        try {
            ...
            const newUi = { ..._state.ui, isInSeclusion: false, seclusionYears: 0, seclusionProgress: 0, seclusionReadyForBreakthrough: false };
            const newLog = [...(_state.eventLog || []), 'You end your seclusion. The world awaits.'];
            return { player: { ..._state.player }, ui: newUi, eventLog: newLog };
        }
        catch (e) {
            logger_1.logger.error('exitSeclusion failed', e);
            return _state;
        }
    }),
    seclusionTick: (hours = 1) => set(_state => {
        try {
            ...
            if (!seclusion)
                return _state;
            const effects = seclusion.tick({ player: _state.player, world: _state.world, story: _state.story, ui: _state.ui, systems: _state.systems });
                ...
                effects.events.forEach((e) => {
                    if (e.type === 'rare_source_found') {
                        ...
                        try {
                            ...
                            const frag = { id: 'manual_fragment', name: 'Manual Fragment', description: 'A fragment', rank: "H", effects: { cultivationSpeed: 1.01 } };
                        ...
                        try {
                            ...
                            const seclusionTechniques = playerTechniqueIds.map((id) => {
                            ...
                            combatToSet = new CombatSystem_1.CombatSystem(playerPart, [rivalTemplate], store, store.rivalSystem, { type: 'normal' });
                        ...
                        try {
                            ...
                            if (iv && typeof iv.enqueueThoughtAuto === 'function') {
                                iv.enqueueThoughtAuto('comprehension', 'player', 'My understanding deepens — a comprehension breakthrough!', { importance: 2, ttlMs: 20000 });
                            }
                        }
                        catch (e) { /* non-fatal */ }
                    }
            ...
            try {
                ...
                if (!alreadyHasManual) {
                    ...
                    const conf = seclusionPath_1.SECLUSION_CONFIG || null;
                    ...
                    if ((conf && typeof conf.playtestMultiplier === 'number' && conf.playtestMultiplier >= 50) || (typeof seededRng_1.runtimeRng === 'function' && (0, seededRng_1.runtimeRng)() < rareThreshold)) {
                        const allManuals = manuals_1.ALL_MANUALS || [];
                        const candidate = allManuals.find((m) => m.rank === "H") || allManuals[0];
                        const manualItem = candidate ? { id: candidate.id || 'manual_fragment', name: candidate.name || 'Manual Fragment', description: candidate.description || 'A fragment', rank: candidate.rank || "H", effects: candidate.effects || { cultivationSpeed: 1.02 } } : { id: 'manual_fragment', name: 'Manual Fragment', description: 'A fragment', rank: "H", effects: { cultivationSpeed: 1.01 } };
                        newPlayer.manuals = [...(newPlayer.manuals || []), manualItem];
                        if (!eventLogs.includes('You discover a rare manual/source in seclusion!'))
                            eventLogs.push('You discover a rare manual/source in seclusion!');
                    }
                }
            }
            ...
            try {
                const realmKey = (0, playerHelpers_1.getPlayerRealmKey)(_state.player);
                const realm = cultivationRealms_1.CULTIVATION_REALMS[realmKey];
                if (realm) {
            ...
            logger_1.logger.error('seclusionTick failed', e);
            return _state;
        }
    }),
    ...
    performSeclusionStudy: (intensity = 1) => set(_state => {
        try {
            ...
            if (!seclusion)
                return _state;
            const res = seclusion.performStudy({ player: _state.player, world: _state.world, story: _state.story, ui: _state.ui, systems: _state.systems }, intensity);
            ...
            return { player: newPlayer, world: { ..._state.world, tick: _state.world.tick + 1 }, eventLog: nextLog };
        }
        catch (e) {
            logger_1.logger.error('performSeclusionStudy failed', e);
            return _state;
        }
    }),
    ...
    // Data helpers
    getRaceBackgrounds: (race) => raceBackgrounds_1.RACE_BACKGROUNDS[race] || [],
    ...
    _computeMinorStageProgress: () => {
        try {
            const s = get();
            const realmKey = (0, playerHelpers_1.getPlayerRealmKey)(s.player);
            const realm = cultivationRealms_1.CULTIVATION_REALMS[realmKey];
            if (!realm)
                return 0;
    ...
    stopAutoCultivation: () => {
        const id = get().ui._cultivationTimerId;
        try {
            if (id != null)
                clearInterval(id);
        }
    ...
    startCultivation: (minutes) => {
        try {
            ...
            set(state => ({ player: { ...state.player, isCultivating: true, cultivationStartTick: state.world.tick }, ui: { ...state.ui, cultivationProgress: state.player.cultivationProgressPercent ?? 0 } }));
    ...
    stopCultivation: () => {
        ...
        set(state => ({ player: { ...state.player, isCultivating: false, cultivationStartTick: undefined }, ui: { ...state.ui, cultivationProgress: state.player.cultivationProgressPercent ?? 0 } }));
    ...
    runCultivationSession: (minutes) => {
        ...
        try {
            ...
            const realmKey = (0, playerHelpers_1.getPlayerRealmKey)(store.player) || 'mortal';
            ...
            const modsBase = (0, cultivationMechanics_1.getApplicableModifiers)([], [], store.player.bloodline?.id || store.player.bloodline?.id, store.player.physique?.id || store.player.physique?.id, equippedManualId);
            const modsExtra = (0, cultivationMechanics_1.getBackgroundPassiveModifiersFromPlayer)(store.player);
            const mods = [...modsBase, ...modsExtra];
            const safeMinutes = Math.max(1, Math.floor(minutes || 1));
            const session = (0, cultivationMechanics_1.generateCultivationSession)(realmKey, talent, safeMinutes, mods);
            ...
            try {
                const p = get().player;
                const idx = Array.isArray(p.activeBuffs) ? p.activeBuffs.findIndex((b) => b && (b.id === 'qi_purification_1' || b.specialEffectId === 'qi_purification_1')) : -1;
                if (idx > -1) {
                    qiGained = Math.floor(qiGained * 2);
                    get().addEventLog('Your meridians feel pure, enhancing your cultivation!');
                    // consume the buff
                    set(state => {
                        const buffs = Array.isArray(state.player.activeBuffs) ? [...state.player.activeBuffs] : [];
                        buffs.splice(idx, 1);
                        return { player: { ...state.player, activeBuffs: buffs } };
                    });
                }
            }
            ...
            const currentRealmKey = (0, playerHelpers_1.getPlayerRealmKey)(store.player);
            const realm = cultivationRealms_1.CULTIVATION_REALMS[currentRealmKey];
            ...
            set(state => ({
                player: {
                    ...
                    ...{ fatigue: (state.player.fatigue || 0) + safeMinutes * 0.1 + extraFatigue }
                },
                world: { ...state.world, tick: state.world.tick + Math.max(1, Math.floor(safeMinutes / 60)) },
                ui: { ...state.ui, cultivationProgress: newPercent }
            }));
            ...
            if (newPercent >= 100) {
                ...
                set(state => ({ player: { ...state.player, isCultivating: false }, ui: { ...state.ui, cultivationProgress: 100 } }));
                get().addEventLog('Your cultivation has reached the threshold. Prepare for the tribulation or attempt breakthrough.');
            }
        }
        catch (e) {
            logger_1.logger.error('runCultivationSession failed', e);
    ...
    runTrainingSession: (minutes, method) => {
        try {
            const safeMinutes = Math.max(1, Math.floor(minutes || 1));
            const realm = (0, playerHelpers_1.getPlayerRealmKey)(get().player) || 'mortal';
            // cooldowns (seconds) per method to match existing APIs
            const cooldowns = { comprehend: 1, body: 1, meditate: 1, martial: 1 };
            const last = get().player.trainingTimestamps?.[method] ?? 0;
            const now = Date.now();
            if (now - last < (cooldowns[method] || 1) * 1000)
                return { success: false, message: 'Training is on cooldown' };
            ...
            set(state => {
                // ensure structures
                state.player.trainingTimestamps = { ...(state.player.trainingTimestamps || {}), [method]: Date.now() };
                if (method === 'comprehend') {
                    (0, proficiency_1.addProficiency)(state, 'comprehension', realm, points);
                    state.player.daoComprehension = (state.player.daoComprehension || 0) + Math.max(0, Math.floor(points / 10));
                    state.player.recentStatDelta = { ...state.player.recentStatDelta, insight: (state.player.recentStatDelta?.insight || 0) + Math.max(0, Math.floor(points / 20)) };
                }
                else if (method === 'body') {
                    (0, proficiency_1.addProficiency)(state, 'strength', realm, points);
                    (0, proficiency_1.addProficiency)(state, 'endurance', realm, Math.max(5, Math.floor(points / 2)));
                    ...
                    state.player.recentStatDelta = { ...state.player.recentStatDelta, atk: (state.player.recentStatDelta?.atk || 0) + gain, def: (state.player.recentStatDelta?.def || 0) + gain };
                }
                else if (method === 'meditate') {
                    (0, proficiency_1.addProficiency)(state, 'willPower', realm, points);
                    state.player.currentQi = Math.min((state.player.currentQi || 0) + Math.max(5, Math.floor(points / 2)), state.player.maxQi || 100);
                }
                else if (method === 'martial') {
                    (0, proficiency_1.addProficiency)(state, 'strength', realm, Math.max(6, Math.floor(points * 0.8)));
                    (0, proficiency_1.addProficiency)(state, 'speed', realm, Math.max(4, Math.floor(points * 0.6)));
                    state.player.baseStats = state.player.baseStats || {};
                    state.player.stats = state.player.stats || {};
                    ...
                    state.player.recentStatDelta = { ...state.player.recentStatDelta, atk: (state.player.recentStatDelta?.atk || 0) + atkGain, speed: (state.player.recentStatDelta?.speed || 0) + speedGain };
                }
                // advance world time similar to cultivation sessions (1 tick per hour)
                state.world = { ...(state.world || {}), tick: (state.world?.tick || 0) + Math.max(1, Math.floor(safeMinutes / 60)) };
                // set a quick UI training progress snapshot (not animated here)
                const percent = Math.min(100, Math.round((safeMinutes / 60) * 100));
                state.ui = { ...(state.ui || {}), trainingProgress: percent };
                return { player: { ...state.player }, world: { ...state.world }, ui: { ...state.ui } };
            });
            get().addEventLog(`You trained (${method}) for ${safeMinutes} minute(s) and improved your abilities.`);
            return { success: true };
        }
        catch (e) {
            logger_1.logger.error('runTrainingSession failed', e);
    ...
    startTrainingById: (trainingId) => {
        try {
            ...
            if (result && result.ok)
                get().addEventLog?.(`Started training: ${trainingId}`);
            else if (result && result.error)
                get().addEventLog?.(`Failed to start training: ${String(result.error)}`);
            return result;
        }
        catch (e) {
            logger_1.logger.error('startTrainingById failed', e);
            return { error: 'failed' };
        }
    },
    tickTrainingQueue: (nowTick) => {
        try {
            set(state => {
                const now = typeof nowTick === 'number' ? nowTick : ((state.world && state.world.tick) || Date.now());
                try {
                    TrainingEngine.tickTraining(state.player, now);
                }
                catch (e) {
                    logger_1.logger.debug('tickTrainingQueue inner', e);
                }
                return { player: { ...state.player } };
            });
        }
        catch (e) {
            logger_1.logger.error('tickTrainingQueue failed', e);
        }
    },
    resolveTrainingQueue: (nowTick) => {
        try {
            let result;
            set(state => {
                const now = typeof nowTick === 'number' ? nowTick : ((state.world && state.world.tick) || Date.now());
                try {
                    result = TrainingEngine.resolveTraining(state.player, now);
                }
                catch (e) {
                    result = { error: e };
                }
            ...
            if (result && result.success)
                get().addEventLog?.('Training completed.');
            else if (result && result.success === false)
                get().addEventLog?.('Training failed.');
            return result;
        }
        catch (e) {
            logger_1.logger.error('resolveTrainingQueue failed', e);
            return { error: 'failed' };
        }
    },
    getTrainingDefinition: (trainingId) => {
        try {
            return TrainingEngine.getTraining(trainingId);
        }
        catch (e) {
            logger_1.logger.debug('getTrainingDefinition failed', e);
            return null;
        }
    },
    computeTrainingSuccessModifier: (trainingId) => {
        try {
            ...
            logger_1.logger.debug('computeTrainingSuccessModifier failed', e);
            return 1;
        }
    },
    // Domain mutators (persist automatically via systems)
    addDomainResource: (key, amount) => set(state => ({ systems: { ...state.systems, ...{ domain: DomainSystem_1.DomainSystem.addResource((state.systems.domain || DomainSystem_1.DomainSystem.defaultState()), key, amount) } } })),
    setDomainUnlock: (key, value = true) => set(state => ({ systems: { ...state.systems, ...{ domain: DomainSystem_1.DomainSystem.setUnlock((state.systems.domain || DomainSystem_1.DomainSystem.defaultState()), key, value) } } })),
    addDomainBonus: (key, amount) => set(state => ({ systems: { ...state.systems, ...{ domain: DomainSystem_1.DomainSystem.addBonus((state.systems.domain || DomainSystem_1.DomainSystem.defaultState()), key, amount) } } })),
    gainDomainXP: (amount) => set(state => ({ systems: { ...state.systems, ...{ domain: DomainSystem_1.DomainSystem.gainXP((state.systems.domain || DomainSystem_1.DomainSystem.defaultState()), amount) } } })),
    recruitTerritoryGarrison: (territoryId, hireCount, quality) => {
        try {
            set(state => {
                const ds = state.systems.domain || DomainSystem_1.DomainSystem.defaultState();
                const domain = new DomainSystem_1.DomainSystem(ds);
                // perform recruitment; return value intentionally ignored
                domain.recruitGarrison(territoryId, hireCount, quality);
                return { systems: { ...state.systems, ...{ domain: domain.serialize() } } };
            });
            try {
                get().addEventLog?.(`Recruited ${hireCount} troops for ${territoryId}.`);
            }
            catch (e) {
                void e;
            }
            ...
            try {
                get().addEventLog?.(`Failed to recruit troops for ${territoryId}.`);
            }
            catch (er) {
                void er;
            }
    ...
    // Mini-game API
    startMiniGame: (id, difficulty = 'easy') => {
        const store = get();
    ...
    completeMiniGame: (result) => {
        ...
        if (!active)
            return null;
        const outcome = sharedMiniGameSystem.endSession(active, result);
    ...
    travelTo: (locationIdOrOptions) => {
        try {
            ...
            if (typeof locationIdOrOptions === 'string' || locationIdOrOptions === null) {
                ...
                durationTicks = Math.max(0, Math.floor(locationIdOrOptions.durationTicks || 0));
            }
            if (durationTicks > 0) {
                // Depart now
            ...
            logger_1.logger.error('travelTo failed', e);
        }
    },
    // New travel APIs (tick-driven, non-real-time)
    isTraveling: () => {
        const s = get();
        return !!(s.player && s.player.activeTravel);
    },
    startTravel: (toNodeId, opts = {}) => {
        try {
            const s = get();
            const from = opts.fromNodeId ?? s.player.currentMapNode ?? null;
            ...
            try {
                const nodes = s.player.mapNodes || [];
                const edges = s.player.mapEdges || [];
                if (nodes && nodes.length > 0 && edges && edges.length > 0 && from && toNodeId) {
                    const res = (0, travelPath_1.findShortestPath)(nodes, edges, from, toNodeId);
                    if (res) {
            ...
            if (durationSeconds > 0)
                meta.realTimeDurationSeconds = durationSeconds;
            // If destination has a gate and player can pay, auto-use fast-travel (teleport)
            try {
                const destNode = (s.player.mapNodes || []).find((n) => n.id === toNodeId);
            ...
            try {
                const nodes = get().player.mapNodes || [];
                const edges = get().player.mapEdges || [];
                if (nodes && edges && opts.fromNodeId != null && toNodeId) {
                    const res = (0, travelPath_1.findShortestPath)(nodes, edges, (opts.fromNodeId || get().player.currentMapNode || null), toNodeId);
                    if (res && res.path && res.path.length > 1) {
                        ...
                        for (let i = 0; i < res.path.length - 1; i++) {
                            ...
                            if (e) {
                                ...
                                const pendingItem = { epochMs, from: a, to: b, encounter: encObj };
                        ...
                        if (pending.length > 0)
                            meta.pendingEncounters = pending;
                        // expose aggregated cost if any
                        const aggCost = {};
                        for (const _p of res.path || []) {
                        ...
                        for (const ed of edges || []) {
                            ...
                            if (!onPath)
                                continue;
                            for (const k of Object.keys(ed.cost || {}))
                                aggCost[k] = (aggCost[k] || 0) + (ed.cost?.[k] || 0);
                        }
                        if (Object.keys(aggCost).length > 0)
                            meta.plannedCost = aggCost;
                    }
                }
            }
            catch (e) {
            ...
            try {
                set(state => ({ player: { ...state.player, lastPlannedTravel: { toNodeId: toNodeId, opts } } }));
            }
            catch (e) {
                logger_1.logger.warn('useGameStore init helper failed', e);
            }
            // Build the active travel object to store on player state
            const active = { fromNodeId: from, toNodeId: toNodeId, startedAtTick: startedAt, arrivalTick: arrival, durationTicks: durationTicks, mode: opts.mode || 'walk', costPaid: opts.costPaid || {}, meta };
            ...
            logger_1.logger.error('startTravel failed', e);
            return null;
        }
    },
    cancelTravel: (_refund) => {
        try {
            const s = get();
            const active = s.player.activeTravel;
            if (!active)
                return false;
            ...
            logger_1.logger.error('cancelTravel failed', e);
            return false;
        }
    },
    ...
    processTravelTick: () => {
        try {
            ...
            if (now >= (active.arrivalTick || 0)) {
                // Arrive
                set(state => ({ player: { ...state.player, ...{ currentMapNode: active.toNodeId || '', unlockedMapNodes: Array.from(new Set([...(state.player.unlockedMapNodes || []), ...(active.toNodeId ? [active.toNodeId] : [])])) }, activeTravel: null } }));
            ...
            logger_1.logger.error('processTravelTick failed', e);
            return false;
        }
    },
    // Dialogue system APIs
    startDialogue: (dialogueOrId) => {
        try {
            const DIALOGUES = dialogues_1.DIALOGUES;
            ...
            if (dlg.timedChoice) {
                try {
                    return get().startTimedChoice(dlg, dlg.timedChoice.timeoutMs ?? 10000, dlg.timedChoice.defaultChoiceIndex ?? 0);
                }
                catch (e) {
            ...
            set(_state => ({ currentDialogue: dlg, currentLineIndex: 0 }));
            return { success: true };
        }
        catch (e) {
            logger_1.logger.error('startDialogue failed', e);
    ...
    advanceDialogue: () => {
        ...
        if (!d)
            return { success: false, reason: 'no_dialogue' };
    ...
    chooseDialogueChoice: (choiceIndex) => {
        ...
        try {
            get().appendTranscript?.({ type: 'choice', text: String(choice.text || ''), meta: { dialogueId: d.id, lineIndex: g.currentLineIndex, choiceIndex } });
        }
        catch (e) {
            void e;
        }
    ...
    startTimedChoice: (dialogueOrId, timeoutMs = 10000, defaultChoiceIndex = 0) => {
        try {
            // clear any existing timer
            try {
                get()._clearTimedChoice?.();
            }
            catch (e) { /* ignore */ }
            const DIALOGUES = dialogues_1.DIALOGUES;
            const dlg = typeof dialogueOrId === 'string' ? DIALOGUES.find(d => d.id === dialogueOrId) : dialogueOrId;
            ...
            const now = Date.now();
            set(_state => ({ currentDialogue: dlg, currentLineIndex: 0, _timedChoiceMeta: { startAt: now, timeoutMs, defaultChoiceIndex } }));
            ...
            const payload = {
                ...
                _clearTimedChoice: () => {
                    try {
                        clearTimeout(get()._activeTimedChoiceTimer);
                    }
                    catch (e) {
                        void e;
                    }
                    try {
                        set({ _activeTimedChoiceTimer: null, _timedChoiceMeta: null });
                    }
                    catch (e) {
                        void e;
                    }
    ...
    appendTranscript: (entry) => {
        try {
            set(state => ({ _transcript: [...(state._transcript || []), { ...entry, ts: Date.now() }] }));
            // debounce persist
            try {
                get()._debouncedPersistTranscript?.();
            }
            catch (e) {
                void e;
            }
    ...
    startMonologueByTag: (tag) => {
        try {
            ...
            if (!candidates || !candidates.length)
                return { success: false };
            const cooldowns = get()._monologueCooldowns || {};
            const defaultCd = get()._defaultMonologueCooldownMs || 60000;
            ...
            if (weighted.length === 1)
                ...
                if (!chosen)
                    chosen = weighted[weighted.length - 1].p;
            }
            if (!chosen)
                return { success: false };
            // record cooldown
            try {
                ...
                set((_state) => ({ _monologueCooldowns: { ...(_state._monologueCooldowns || {}), [chosen.id]: Date.now() } }));
                void _cd;
            }
            catch (e) { /* ignore */ }
    ...
    getVisibleStats: () => {
        ...
        const atkBonus = proficiency_1.computeStatFromProficiency(store, 'strength') || 0;
        const speedBonus = proficiency_1.computeStatFromProficiency(store, 'speed') || 0;
        const willBonus = proficiency_1.computeStatFromProficiency(store, 'willPower') || 0;
        ...
        return {
            hp: p.hp ?? p.maxHp ?? 0,
            qi: (0, statTiers_1.tierLabelFor)(p.stats?.qi ?? p.qi ?? 0),
            ...
            atk: (0, statTiers_1.tierLabelFor)(atkValue),
            def: (0, statTiers_1.tierLabelFor)(defValue),
            speed: (0, statTiers_1.tierLabelFor)(speedValue),
            willPower: { label: String((p.willPower || 0) + willBonus), value: (p.willPower || 0) + willBonus }
        };
    },
    // Debug
    attachToWindow: () => {
    ...
    // Life-phase APIs
    startLifePhase: (type, title, karmicModifiers) => {
        try {
            ...
            logger_1.logger.error('startLifePhase failed', e);
            return null;
        }
    },
    endLifePhase: () => {
        try {
            ...
            set(state => ({ systems: { ...state.systems, lifePhase: sharedLifePhaseSystem.getState() } }));
            if (ended)
                get().addEventLog(`Life phase ended: ${ended.title || ended.id}`);
            return ended;
        }
        catch (e) {
            logger_1.logger.error('endLifePhase failed', e);
            return null;
        }
    },
    recordLifeEvent: (e) => {
        try {
            ...
            logger_1.logger.error('recordLifeEvent failed', err);
        }
    },
    renderLifeNovel: () => {
        try {
            ...
            logger_1.logger.error('renderLifeNovel failed', e);
            return '';
        }
    },
    // Mentor teaching system integration
    getAvailableTeachingsForMentor: (mentorId) => {
        const store = get();
    ...
    getMentorCooldownRemaining: (mentorId) => {
        const store = get();
    ...
    attemptMentorTeaching: (mentorId, teachingId) => {
        const store = get();
            ...
            set(state => ({
                systems: {
                    ...
                    teachingProgress: {
                        ...
                        [teachingId]: res.updatedProgress
                    }
                }
            }));
        }
        ...
        if (!res.result.success && res.result.penaltyConsequences?.cooldownRemaining !== undefined) {
            const remaining = res.result.penaltyConsequences.cooldownRemaining;
    ...
    finalizeCharacterCreation: (payload) => {
        const store = get();
        const safePayload = payload || { name: store.player.name || 'Player', gender: store.player.gender || 'Male' };
        const { name, gender } = safePayload;
        // Basic validation
        if (!name || !gender) {
        ...
        let selectedRace = safePayload.race || store.player.race;
        let selectedBackground = safePayload.selectedBackground || store.player.background;
        ...
        if (selectedBackground && !selectedRace) {
            ...
            for (const r of Object.keys(raceBackgrounds_1.RACE_BACKGROUNDS)) {
                if (raceBackgrounds_1.RACE_BACKGROUNDS[r].some(b => b.id === bgId)) {
                    selectedRace = r;
                    break;
                }
            }
        ...
        if (!selectedRace || !selectedBackground) {
            ...
            try {
                const r = (0, seededRng_1.runtimeRng)();
                if (r < 0.8 && races.includes('Human')) {
                    ...
                    selectedRace = pool[Math.floor((0, seededRng_1.runtimeRng)() * pool.length)];
                }
            }
            catch (e) {
                selectedRace = races[Math.floor((0, seededRng_1.runtimeRng)() * races.length)];
            }
            const backgrounds = raceBackgrounds_1.RACE_BACKGROUNDS[selectedRace];
            // Synchronous weighted sampler using deterministic runtimeRng
            try {
                const selectFromWeightedList = (list, getWeight) => {
                    const weights = list.map(getWeight).map(w => (Number.isFinite(w) && w > 0 ? w : 0));
                    const total = weights.reduce((a, b) => a + b, 0);
                    if (!Number.isFinite(total) || total <= 0) {
                        // fallback to uniform if no positive weights
                        return list[Math.floor((0, seededRng_1.runtimeRng)() * list.length)];
                    }
                    let r = (0, seededRng_1.runtimeRng)() * total;
                ...
                const rarityWeights = rarity_1.RARITY_WEIGHTS;
                const getWeight = (b) => {
                    if (typeof b.startChance === 'number')
                        return b.startChance;
                    const r = String(b.rarity || "H");
                    return rarityWeights[r] || 1;
                };
                if (selectedRace === 'Human') {
                    const humanHasWeights = backgrounds.some((b) => typeof b.startChance === 'number');
                    selectedBackground = humanHasWeights
                        ? selectFromWeightedList(backgrounds, (b) => (typeof b.startChance === 'number' ? b.startChance : 0))
                        : selectFromWeightedList(backgrounds, getWeight);
                }
                else {
                    selectedBackground = selectFromWeightedList(backgrounds, getWeight);
                }
            }
            catch (e) {
                selectedBackground = backgrounds[Math.floor((0, seededRng_1.runtimeRng)() * backgrounds.length)];
            }
        }
        // Apply background effects (ensure non-null selectedBackground)
        const fallbackBg = { id: 'unknown', name: 'Unknown', effects: {}, rarity: "H" };
        const bg = selectedBackground || fallbackBg;
        const bgEffects = bg.effects || {};
        let next = { ...store.player };
        next.name = name;
        next.gender = gender;
        ...
        // Currency/resources
        if (typeof bgEffects.yuan === 'number') {
            next.yuan = (next.yuan || 0) + bgEffects.yuan;
        }
        if (bgEffects.spiritStones) {
            next.spiritStones = { ...(next.spiritStones || { low: 0, mid: 0, high: 0 }), ...bgEffects.spiritStones };
        }
        // Skill level bumps (supports both number and object with level)
        if (bgEffects.skills) {
            next.skills = { ...next.skills };
            Object.entries(bgEffects.skills).forEach(([skill, val]) => {
        ...
        Object.entries(bgEffects).forEach(([key, val]) => {
            if (val && typeof val === 'object' && 'level' in val && next.skills?.[key]) {
                next.skills[key] = {
                    ...next.skills[key],
                    level: Math.max(next.skills[key].level, val.level || 0)
                };
            }
        });
        // Background passives (apply via passive registry; tolerate missing registrations)
        try {
            (0, passiveRegistry_1.ensureGeneratedPassives)(); // kick off background load of generated passives (non-blocking)
            const passives = bgEffects.passives;
            if (passives && typeof passives === 'object') {
                const appliedIds = [];
                Object.entries(passives).forEach(([pid, cfg]) => {
                    const lvl = (cfg && typeof cfg === 'object' && typeof cfg.level === 'number') ? Math.max(1, cfg.level) : 1;
                    for (let i = 0; i < lvl; i++) {
                        try {
                            next = (0, passiveRegistry_1.applyPassiveToPlayer)(next, pid, { source: 'background', level: lvl, index: i });
                        }
                        catch (e) { /* ignore per-passive failures */ }
                    }
                    if (!next.passiveIds)
                        next.passiveIds = [];
                    if (!next.passiveIds.includes(pid))
                        next.passiveIds.push(pid);
                    appliedIds.push(pid);
                });
                if (appliedIds.length) {
                    try {
                        get().addEventLog(`Background passives gained: ${appliedIds.join(', ')}`);
                    }
                    catch (e) { /* ignore */ }
                }
            }
        ...
        // Reputation
        if (bgEffects.reputation) {
            next.reputation = { ...(next.reputation || {}), ...bgEffects.reputation };
        }
        ...
        try {
            ...
            // Record birth event
            sharedLifePhaseSystem.recordEvent({ id: 'birth', title: 'Birth', description: 'You are born into the world.', context: { playerName: next.name, region: next.currentLocationId || 'homeland' } });
            // Record awakening based on daoPrinciple or talent
            sharedLifePhaseSystem.recordEvent({ id: 'awakening', title: 'Awakening', description: 'The Dao stirs within.', context: { playerName: next.name, region: next.currentLocationId || 'homeland', daoConcept: next.daoPrinciple || next.talentId || 'mortal spirit' } });
            // Record chosen path/background
            sharedLifePhaseSystem.recordEvent({ id: 'choosing_path', title: 'Choosing Path', description: `Chose background ${bg.name}`, context: { playerName: next.name, pathName: (bg && bg.name) || 'Unknown', villageName: (bg && bg.villageName) || undefined } });
            ...
            try {
                get().addEventLog('LifePhase initialization failed.');
            }
            catch (err) { /* ignore */ }
        }
    ...
    showToast: (message, duration = 3000) => {
        try {
            ...
            setTimeout(() => {
                try {
                    exports.useGameStore.setState({ toast: null });
                }
                catch (e) { /* ignore */ }
            }, duration);
        }
    ...
    assignRandomBloodline: () => set(state => {
        const availableBloodlines = data_1.ALL_BLOODLINES.filter((b) => b.rarity === "H" ||
            (b.rarity === "G" && (0, seededRng_1.runtimeRng)() < 0.3) ||
            (b.rarity === "F" && (0, seededRng_1.runtimeRng)() < 0.1));
        const randomBloodline = availableBloodlines[Math.floor((0, seededRng_1.runtimeRng)() * availableBloodlines.length)];
        return {
            player: {
                ...state.player,
                bloodline: {
    ...
    assignRandomPhysique: () => set(state => {
        const availablePhysiques = physiques_1.PHYSIQUES.filter(p => p.rarity === "H" ||
            (p.rarity === "G" && (0, seededRng_1.runtimeRng)() < 0.2) ||
            (p.rarity === "F" && (0, seededRng_1.runtimeRng)() < 0.05));
        const randomPhysique = availablePhysiques[Math.floor((0, seededRng_1.runtimeRng)() * availablePhysiques.length)];
        return {
            player: {
                ...state.player,
    ...
    trainWithManual: (manualId) => {
        const manual = (0, manuals_1.getManualByIdScaled)(manualId, (0, playerHelpers_1.getPlayerRealmKey)(get().player));
        if (!manual)
            return { success: false, message: 'Manual not found' };
        ...
        const hasInManuals = owned.some((m) => m.id === manualId);
        const invIndex = inv.findIndex(i => i?.meta?.type === 'manual' && i.id === manualId);
        if (!hasInManuals && invIndex === -1)
            return { success: false, message: 'You do not possess that manual' };
        set(state => {
            // If in manuals list, remove one occurrence
            if ((state.player.manuals || []).some((m) => m.id === manualId)) {
                state.player.manuals = (state.player.manuals || []).filter((m) => m.id !== manualId);
            }
            else {
                // Consume from inventory (decrement quantity if present)
                const idx = (state.player.inventory || []).findIndex((it) => it?.meta?.type === 'manual' && it.id === manualId);
                if (idx !== -1) {
            ...
            const targets = [];
            if (effects.atk)
                targets.push('strength');
            if (effects.speed)
                targets.push('speed');
            if (effects.soulWill || effects.soulWill || effects.soulWill)
                targets.push('willPower');
            if (effects.daoHeart || effects.martialMastery || effects.comprehension)
                targets.push('comprehension');
            // If no explicit targets, default to comprehension training
            if (targets.length === 0)
                targets.push('comprehension');
            // Grant proficiency points scaled by manual rank/effects
            const realm = (0, playerHelpers_1.getPlayerRealmKey)(state.player) || 'mortal';
            const tier = (0, proficiency_1.realmToTier)(realm);
            const pointsPerEffect = Math.max(5, Math.floor((effects.cultivationSpeed || 20) / (tier === 'early' ? 1 : tier === 'mid' ? 1.5 : 2)));
            for (const t of targets) {
                (0, proficiency_1.addProficiency)(state, t, realm, pointsPerEffect);
                ...
                if (t === 'strength') {
                    ...
                    state.player.recentStatDelta = { ...state.player.recentStatDelta, atk: (state.player.recentStatDelta?.atk || 0) + visibleGain };
                }
                else if (t === 'speed') {
                    ...
                    state.player.recentStatDelta = { ...state.player.recentStatDelta, speed: (state.player.recentStatDelta?.speed || 0) + visibleGain };
                }
                else if (t === 'willPower') {
                    state.player.willPower = (state.player.willPower || 0) + visibleGain;
                }
                else if (t === 'comprehension') {
                    state.player.stats = state.player.stats || {};
                    state.player.stats.insight = (state.player.stats.insight || 0) + visibleGain;
                    state.player.recentStatDelta = { ...state.player.recentStatDelta, insight: (state.player.recentStatDelta?.insight || 0) + visibleGain };
                }
            }
            return { player: { ...state.player } };
        });
    ...
    trainComprehendManual: (manualId) => {
        ...
        try {
            ...
            if (manualId) {
                const p = get().player;
                const hasInManuals = Array.isArray(p.manuals) && p.manuals.some((m) => m.id === manualId);
                const invHas = Array.isArray(p.inventory) && p.inventory.some((it) => (it?.meta?.type === 'manual' && it.id === manualId) || it.id === manualId);
                if (!hasInManuals && !invHas)
                    return { success: false, message: 'You do not possess that manual' };
            }
            return get().runTrainingSession(60, 'comprehend');
        }
        catch (e) {
            return { success: false, message: 'Training failed' };
        }
    ...
    trainBody: () => {
        ...
        try {
            return get().runTrainingSession(60, 'body');
        }
        catch (e) {
    ...
    trainMeditate: () => {
        try {
            return get().runTrainingSession(60, 'meditate');
        }
        catch (e) {
    ...
    trainMartial: () => {
        try {
            return get().runTrainingSession(60, 'martial');
        }
        catch (e) {
            return { success: false, message: 'Training failed' };
        }
    ...
    gainSkillExp: (skillId, exp) => set(state => {
        ...
        if (leveled > 0) {
            ...
            const have = new Set(Array.isArray(player.techniques) ? player.techniques : []);
            list.forEach(u => { if ((updated.level || 0) >= u.level)
                have.add(u.techId); });
            player.techniques = Array.from(have);
        }
        return { player };
    }),
    evolveSkill: (baseSkillId, evolutionPath) => {
        // Placeholder
        get().addEventLog(`Skill ${baseSkillId} evolved to ${evolutionPath}`);
    },
    checkRealmBreakthrough: () => {
        const store = get();
        const realmData = cultivationRealms_1.CULTIVATION_REALMS[(0, playerHelpers_1.getPlayerRealmKey)(store.player)];
        if (!realmData)
            return;
        ...
        if (currentQi >= requiredQi) {
            ...
            const startRealm = (0, playerHelpers_1.getPlayerRealmKey)(store.player) || 'mortal';
            let finalRealm = startRealm;
            // hasManual check removed (not used here)
            try {
                let candidate = (0, cultivationRealms_1.getNextRealm)(finalRealm);
                while (candidate) {
                    ...
                    const candidateReq = cultivationRealms_1.CULTIVATION_REALMS[candidate]?.qiRequirement || Number.MAX_SAFE_INTEGER;
                    if (currentQi >= candidateReq) {
                        finalRealm = candidate;
                        candidate = (0, cultivationRealms_1.getNextRealm)(finalRealm);
                        continue;
                    }
                    break;
                }
                ...
                finalRealm = (0, cultivationRealms_1.getNextRealm)(startRealm) || startRealm;
            }
            if (finalRealm && finalRealm !== startRealm) {
                ...
                try {
                    ...
                    while (iter !== finalRealm) {
                        const next = (0, cultivationRealms_1.getNextRealm)(iter);
                        if (!next)
                            break;
                        lifespanBonusSum += (cultivationRealms_1.CULTIVATION_REALMS[iter]?.lifespanBonus || 0);
                        iter = next;
                    }
                }
                catch (e) {
                    lifespanBonusSum = realmData.lifespanBonus || 0;
                }
                ...
                set(state => ({
                    player: {
                        ...state.player,
                        realm: finalRealm,
                        realmId: cultivationRealms_1.REALM_ORDER.indexOf(finalRealm),
                        currentQi: 0,
                        qiRequired: cultivationRealms_1.CULTIVATION_REALMS[finalRealm].qiRequirement,
                        lifespan: (state.player.lifespan || 150) + (lifespanBonusSum || realmData.lifespanBonus)
                    }
                }));
                get().addEventLog(`Breakthrough successful! Reached ${cultivationRealms_1.CULTIVATION_REALMS[finalRealm].name} realm. Lifespan increased by ${lifespanBonusSum || realmData.lifespanBonus} years.`);
                ...
                try {
                    ...
                    void _ensureInnerVoice;
                    if (iv && typeof iv.enqueueThoughtAuto === 'function') {
                        iv.enqueueThoughtAuto('breakthrough', 'player', `I've broken through to ${cultivationRealms_1.CULTIVATION_REALMS[finalRealm].name}. This changes everything.`, { importance: 3, ttlMs: 30000 });
                    }
                }
                catch (e) {
                    // non-fatal
                ...
                if (reachedTrueImmortal && !get().world.ascended) {
                    try {
                        const gameState = get();
                        const actId = gameState.story.currentAct;
                        const storySystem = gameState.storySystem || sharedStorySystem;
                        const ascensionEventId = 'forced_ascension_true_immortal';
                        const existing = storySystem?.getAvailableEvents?.(gameState)?.some((e) => e.id === ascensionEventId);
                        if (!existing) {
                            const playerHidden = get().player.hiddenFromHeaven || (get().player.providenceVeil || 0) > 0;
                            if (playerHidden) {
                        ...
                        logger_1.logger.warn('Failed to inject ascension event (non-fatal):', e);
                    }
                }
            }
        }
    ...
    setWorldType: (worldType) => set(state => {
        if (state.world.ascended && worldType !== 'immortal')
            return state; // lock after ascension
    ...
    attemptRealmBreakthroughWithConsolidation: (challengeId) => {
        const store = get();
        // Prevent breakthrough attempts while in seclusion
        const isInSeclusionFlag = store.ui.isInSeclusion;
        ...
        const currentRealmId = (0, playerHelpers_1.getPlayerRealmId)(store.player) || 1;
        if (!currentRealmId)
            return false;
        const currentRealmKey = cultivationRealms_1.REALM_ORDER[currentRealmId - 1];
        ...
        const baseStats = {
            ...
            stability: store.player.stability || 0,
        ...
        const skillLevels = Object.fromEntries(Object.entries(store.player.skills || {}).map(([k, v]) => [k, v?.level || 0]));
        ...
        if (result.success && result.newRealm) {
            ...
            const newRealmIndex = cultivationRealms_1.REALM_ORDER.indexOf(newRealmId);
            ...
            try {
                ...
                (0, playerSetters_1.applyRealmToPlayer)(tmpPlayer, newRealmId);
                store.updatePlayerState({
                    realm: (0, playerHelpers_1.getPlayerRealmKey)(tmpPlayer),
                    realmId: (0, playerHelpers_1.getPlayerRealmId)(tmpPlayer),
                    minorStage: 1,
                    currentQi: 0,
                ...
                store.updatePlayerState({
                    realm: newRealmId,
                    realmId: newRealmIndex >= 0 ? newRealmIndex + 1 : (0, playerHelpers_1.getPlayerRealmId)(store.player),
                    minorStage: 1,
                    currentQi: 0,
                    cultivationProgressPercent: 0
                });
            ...
            if (result.insights)
                result.insights.forEach(ins => store.addEventLog(`Gained insight: ${ins}`));
        }
            ...
            Object.entries(result.penalties).forEach(([stat, penalty]) => {
                if (stat === 'qi' || stat === 'currentQi') {
                    ...
                    updates[stat] = Math.max(0, (store.player[stat] || 0) + penalty);
                }
            });
    ...
    removeFromInventoryById: (itemId, qty = 1) => {
        const gs = get();
        const inv = Array.isArray(gs.player.inventory) ? [...gs.player.inventory] : [];
        if (!itemId)
            return;
    ...
    purchaseFromMarket: (marketId, itemId) => {
        const store = get();
    ...
    listMarkets: () => {
        ...
        return Array.isArray(res) ? res.map((m) => ({ ...m, items: Array.isArray(m.items) ? m.items.map((it) => ({ ...it })) : [] })) : [];
    },
    listMarketItems: (marketId) => {
        const store = get();
        const res = sharedMarketSystem.getMarketItems(marketId, store.player) || [];
        return Array.isArray(res) ? res.map((it) => ({ ...it })) : [];
    },
    listActiveAuctions: () => {
        const res = sharedMarketSystem.getActiveAuctions() || [];
        // clone nested auction.item too so UI never holds proxies
        return Array.isArray(res) ? res.map((a) => ({ ...a, item: a?.item ? { ...a.item } : a.item, bidHistory: Array.isArray(a?.bidHistory) ? a.bidHistory.map((b) => ({ ...b })) : [] })) : [];
    },
    listPlayerMarketInventory: () => {
    ...
    sellToMarket: (marketId, itemId, quantity) => {
        const store = get();
        return sharedMarketSystem.sellToMarket(marketId, itemId, quantity, store.player);
    },
    listItemForAuction: (itemId, startingBid, buyoutPrice) => {
        const store = get();
    ...
    checkMarketRefresh: () => {
        ...
        if (now - lastAuctionRefresh >= auctionRefreshInterval) {
            ...
            if (typeof sharedMarketSystem.initializeMarkets === 'function') {
                sharedMarketSystem.initializeMarkets();
            }
            set(state => ({
                world: {
                    ...state.world,
    ...
    cultivate: () => {
        ...
        try {
            const _call = get()._callOnPlayerActionWithGu;
                ...
                try {
                    ...
                    (async () => {
                        try {
                            const gu = await Promise.resolve().then(() => __importStar(require('../systems/guBranch')));
                            if (gu && typeof gu.onPlayerActionWithGu === 'function')
                                gu.onPlayerActionWithGu(get(), 'cultivate');
                        }
                        catch { /* ignore */ }
                    })();
                }
    ...
    explore: () => {
        ...
        if (playerLevelSafe >= 10)
            encounterChance += 0.03;
        if (playerLevelSafe >= 20)
            encounterChance += 0.02;
        if (playerLevelSafe >= 30)
            encounterChance += 0.02;
        if (playerLevelSafe >= 40)
            encounterChance += 0.03;
        ...
        const encounterRoll = (0, seededRng_1.runtimeRng)();
        if (encounterRoll < encounterChance) {
            ...
            const availableRivals = store.rivalSystem.getAllRivals().filter(r => !r.defeated &&
                store.canEncounterRival(r.id) &&
                r.relationship < 30 // More likely to encounter rivals with neutral/negative relationships
            );
            if (availableRivals.length > 0 && (0, seededRng_1.runtimeRng)() < 0.2) {
                // Trigger rival encounter
                const rival = availableRivals[Math.floor((0, seededRng_1.runtimeRng)() * availableRivals.length)];
                store.addEventLog(`While exploring, you encounter ${rival.name}, ${rival.title}!`);
                store.startRivalEncounter(rival.id);
                try {
                    get()._callOnPlayerActionWithGu?.('explore_rival');
                }
                catch { /* ignore */ }
                return;
            }
            // Otherwise, generate a random enemy encounter
            const enemyLevel = Math.max(1, Math.floor(playerLevelSafe + ((0, seededRng_1.runtimeRng)() - 0.5) * 10)); // ±5 levels from player
            // Determine enemy realm based on level (not currently used directly)
            let _enemyRealm = 'mortal';
            if (enemyLevel >= 10)
                _enemyRealm = 'qi_condensation';
            if (enemyLevel >= 20)
                _enemyRealm = 'foundation_establishment';
            if (enemyLevel >= 30)
                _enemyRealm = 'core_formation';
            if (enemyLevel >= 40)
                _enemyRealm = 'nascent_soul';
            if (enemyLevel >= 50)
                _enemyRealm = 'spirit_transformation';
            void _enemyRealm;
            ...
            const enemyType = enemyTypes[Math.floor((0, seededRng_1.runtimeRng)() * enemyTypes.length)];
            ...
            const ENEMY_MUL = (balance_1.getEnemyBaseMultiplier && typeof balance_1.getEnemyBaseMultiplier === 'function') ? (0, balance_1.getEnemyBaseMultiplier)() : 1;
            const speedMul = (() => {
            ...
            try {
                get()._callOnPlayerActionWithGu?.('explore_enemy');
            }
            catch { /* ignore */ }
            ...
            // Initialize combat
            const combatSystem = new CombatSystem_1.CombatSystem(playerParticipant, [enemy], store, store.rivalSystem, { type: 'normal' });
        ...
        if ((0, seededRng_1.runtimeRng)() < 0.15) { // 15% chance for special event
            const specialEvents = [
                () => {
                    // Find spirit stones
                    const amount = Math.floor((0, seededRng_1.runtimeRng)() * 5) + 1;
                    set(state => ({
                        player: {
                            ...state.player,
                            spiritStones: {
                ...
                () => {
                    // Find yuan
                    const amount = Math.floor((0, seededRng_1.runtimeRng)() * 20) + 5;
            ...
            const specialEvent = specialEvents[Math.floor((0, seededRng_1.runtimeRng)() * specialEvents.length)];
            specialEvent();
        }
        else {
            // Regular exploration message
            const idx = Math.floor((0, seededRng_1.runtimeRng)() * explorationEvents.length);
            ...
            try {
                const mentorEncounterRoll = (0, seededRng_1.runtimeRng)();
                ...
                if (mentorEncounterRoll < MENTOR_ENCOUNTER_CHANCE) {
                    ...
                    Promise.resolve().then(() => __importStar(require('../data/mentor_encounters_runtime'))).then(mod => {
                        try {
            ...
            if (message.includes('ancient ruins')) {
                try {
                    ...
                    const ruinsQuest = {
                        ...
                        status: 'inactive',
                        objectives: [
                            { id: questId + '_collect', type: 'COLLECT_ITEM', description: 'Collect a Ruins Fragment', target: 'ruins_fragment', value: 1, isCompleted: false },
                            { id: questId + '_return', type: 'REPORT_TO_NPC', description: 'Report findings to a local scholar', target: 'reported_ruins_' + questId, value: 1, isCompleted: false, isOptional: false }
                        ],
                        rewards: [{ type: 'item', target: 'basic_qi_gathering', amount: 1, description: 'A worn cultivation manual' }],
                        prerequisites: []
                    };
                    eqSys.addQuest(ruinsQuest);
                    // Auto-activate the quest so the player has it immediately
                    eqSys.activateQuest(questId, { player: store.player, world: store.world, story: store.story, ui: store.ui, systems: store.systems });
    ...
    reportToNpc: (flagKey) => {
        try {
            ...
            try {
                sharedEnhancedQuestSystem.reportToNpc?.(flagKey, { player: get().player, world: get().world, story: get().story, ui: get().ui, systems: get().systems });
            }
            catch (e) {
                // best-effort only
            }
    ...
    saveGame: () => {
        try {
            ...
            const systemsSnapshot = { ...store.systems };
            try {
            ...
            const ok = SaveLoadSystem_1.SaveLoadSystem.saveGame(snapshot);
            if (ok) {
            ...
            logger_1.logger.error('saveGame failed', e);
            try {
                set(state => ({ ui: { ...state.ui, saveInProgress: false } }));
            }
            catch { /* ignore */ }
        ...
        try {
            ...
            if (current.ui && current.ui.saveInProgress) {
                ...
                _pendingLoadQueue.push(() => {
                    try {
                        ...
                        const innerLoaded = SaveLoadSystem_1.SaveLoadSystem.loadGame();
                        if (!innerLoaded) {
                            get().addEventLog('No save found.');
                            return false;
                        }
                        const innerSaveObj = (innerLoaded && innerLoaded.migrated) ? innerLoaded.migrated : innerLoaded;
                        if (!innerSaveObj || !innerSaveObj.gameState) {
                            get().addEventLog('No save found.');
                            return false;
                        }
                        const innerGs = innerSaveObj.gameState;
                        ...
                        try {
                            const snap = innerSaveObj.narrativeSnapshot || (innerGs && innerGs.systems && innerGs.systems.narrativeSnapshot) || null;
                            const engine = (typeof sharedNarrativeEngine !== 'undefined' ? sharedNarrativeEngine : null) || get().narrativeEngine || (innerGs && innerGs.systems && innerGs.systems.narrativeEngine);
                            if (snap && engine) {
                                try {
                        ...
                        get().addEventLog('Queued load failed.');
                        return false;
                    }
                });
                return false;
            }
            const loaded = SaveLoadSystem_1.SaveLoadSystem.loadGame();
            if (!loaded) {
                get().addEventLog('No save found.');
                return false;
            }
            // Normalize: SaveLoadSystem.loadGame may return { raw, migrated } when preserveRaw was used.
            const saveObj = (loaded && loaded.migrated) ? loaded.migrated : loaded;
            if (!saveObj || !saveObj.gameState) {
                get().addEventLog('No save found.');
                return false;
            }
            const gs = saveObj.gameState;
            ...
            try {
                ...
                if (!gs.world.currentWorldType) {
                    ...
                    const realm = (0, playerHelpers_1.getPlayerRealmKey)(gs.player);
            ...
            try {
                const snap = saveObj.narrativeSnapshot || (gs && gs.systems && gs.systems.narrativeSnapshot) || null;
                const engine = (typeof sharedNarrativeEngine !== 'undefined' ? sharedNarrativeEngine : null) || get().narrativeEngine || (gs && gs.systems && gs.systems.narrativeEngine);
                if (snap && engine) {
                    try {
                        engine.karmaHistory = snap.karmaHistory || engine.karmaHistory || [];
                        engine.destinyThreads = snap.destinyThreads || engine.destinyThreads || [];
            ...
            try {
                const lp = (gs && gs.systems && gs.systems.lifePhase) || (saveObj && saveObj.lifePhaseSnapshot) || null;
                if (lp) {
                    try {
                        ...
                        if (typeof sharedLifePhaseSystem.loadState === 'function') {
                            sharedLifePhaseSystem.loadState(lp);
                        }
                        else if (sharedLifePhaseSystem.state !== undefined) {
                            // best-effort: replace internal state
                            sharedLifePhaseSystem.state = lp;
                        }
            ...
            logger_1.logger.error('loadGame failed', e);
            return false;
        }
    },
    ...
    getCurrentAct: () => {
        ...
        return (system.getPersonalizedAct ? system.getPersonalizedAct({ player: store.player, world: store.world, story: store.story, ui: store.ui, systems: store.systems }) : null) || { id: store.story.currentAct, title: store.story.currentAct };
    },
    getActiveQuests: () => {
        const store = get();
        const system = store.storySystem || sharedStorySystem;
        return (system.getActiveQuests ? system.getActiveQuests({ player: store.player, world: store.world, story: store.story, ui: store.ui, systems: store.systems }) : []);
    },
    getAvailableEvents: () => {
        const store = get();
        const system = store.storySystem || sharedStorySystem;
        return (system.getAvailableEvents ? system.getAvailableEvents({ player: store.player, world: store.world, story: store.story, ui: store.ui, systems: store.systems }) : []);
    },
    getStoryDiagnostics: () => {
        const store = get();
        const system = store.storySystem || sharedStorySystem;
        if (system.getDiagnosticsSummary)
            return system.getDiagnosticsSummary();
        return null;
    },
    resetStoryDiagnostics: () => {
        const store = get();
        const system = store.storySystem || sharedStorySystem;
        if (system.resetDiagnostics)
            system.resetDiagnostics();
    },
    triggerStoryEvent: (eventId) => {
        const store = get();
        const system = store.storySystem || sharedStorySystem;
        const evt = (system.triggerEvent ? system.triggerEvent(eventId, { player: store.player, world: store.world, story: store.story, ui: store.ui, systems: store.systems }) : null);
        if (evt) {
    ...
    makeStoryChoice: (eventId, choiceId) => {
        const store = get();
        const system = store.storySystem || sharedStorySystem;
        const success = (system.makeChoice ? system.makeChoice(eventId, choiceId, { player: store.player, world: store.world, story: store.story, ui: store.ui, systems: store.systems }) : false);
        if (success) {
            // Update quests after story choice
            const completed = (system.checkQuestCompletion ? system.checkQuestCompletion({ player: store.player, world: store.world, story: store.story, ui: store.ui, systems: store.systems }) : []);
            if (completed.length) {
    ...
    getQuestProgress: (questId) => sharedEnhancedQuestSystem.getQuestProgress(questId),
    getObjectiveProgress: (questId, objectiveId) => sharedEnhancedQuestSystem.getObjectiveProgress(questId, objectiveId),
    updateObjectiveProgress: (questId, objectiveId, progress) => sharedEnhancedQuestSystem.updateObjectiveProgress(questId, objectiveId, progress),
    // Crafting helpers
    getAvailableCraftingRecipes: () => sharedCraftingSystem.getAvailableRecipes?.(get().player) || [],
    experimentWithIngredients: (ingredientItemIds) => {
        const store = get();
        try {
            ...
            const result = sharedCraftingSystem.experiment?.(ingredients, store.player);
            if (result) {
    ...
    useItem: (inventoryIndex) => {
        ...
        if (item.effects?.heal) {
            const healed = Math.max(0, Math.min(item.effects.heal, (store.player.maxHp || 100) - (store.player.hp || 0)));
        ...
        set(state => ({ player: sharedBuffSystem.processBuffsOnAction ? sharedBuffSystem.processBuffsOnAction(state.player) : state.player }));
    },
    processBuffs: () => {
        set(_state => ({ player: sharedBuffSystem.processBuffsOnTick ? sharedBuffSystem.processBuffsOnTick(_state.player) : _state.player }));
    },
    removeBuff: (buffId) => {
    ...
    checkDailyReset: () => {
        ...
        if (Date.now() - last >= oneDay) {
            ...
            set(state => ({ player: { ...state.player, recentStatDelta: {} } }));
            ...
            try {
                const post = get();
                const alloc = (post.player.cultivationDaysAllocated || 0);
                if (alloc > 0 && Array.isArray(post.player.manuals) && post.player.manuals.length > 0) {
                    const increment = alloc / 365;
                    const prevAcc = post.player.cultivationScheduleAccumulator || 0;
                    ...
                    // Persist accumulator
                    set(state => ({ player: { ...state.player, ...{ cultivationScheduleAccumulator: acc } } }));
                }
            }
            catch (e) {
                // non-fatal
            ...
            if (yearsPassed > 0) {
                set(state => {
                    ...
                    return {
                        ...
                        ui: (isDead ? { ...state.ui, currentScreen: 'death' } : state.ui)
                    };
                });
    ...
    reincarnate: () => {
        ...
        try {
            const ctx = get();
            const inInv = Boolean(ctx.ui?._inInventoryInteraction);
        ...
        const allRaces = Object.keys(raceBackgrounds_1.RACE_BACKGROUNDS);
        const randomRace = allRaces[(0, rng_1.randInt)(allRaces.length, (0, rng_1.getRng)())];
        const raceData = raceBackgrounds_1.RACE_BACKGROUNDS[randomRace];
        const randomBackground = raceData[(0, rng_1.randInt)(raceData.length, (0, rng_1.getRng)())];
        const backgroundData = randomBackground;
        const randomBloodlineIndex = (0, rng_1.randInt)(data_1.ALL_BLOODLINES.length, (0, rng_1.getRng)());
        const bloodlineData = data_1.ALL_BLOODLINES[randomBloodlineIndex];
        const randomPhysiqueIndex = (0, rng_1.randInt)(physiques_1.PHYSIQUES.length, (0, rng_1.getRng)());
        const physiqueData = physiques_1.PHYSIQUES[randomPhysiqueIndex];
        const talentOptions = ['heavenly', 'peerless', 'supreme', 'excellent', 'good', 'average', 'poor', 'trash'];
        const randomTalent = talentOptions[(0, rng_1.randInt)(talentOptions.length, (0, rng_1.getRng)())];
        // Meta-progression carryover snapshot
        const before = get();
        const prevLegend = before.player.legendPoints || 0;
        const prevUnlocks = before.player.metaUnlocks || {};
        const prevInventory = Array.isArray(before.player.inventory) ? before.player.inventory : [];
        const rarityOrder = { common: 1, uncommon: 2, rare: 3, epic: 4, legendary: 5, mythical: 6, transcendent: 7 };
        ...
        if (carryCount > 0 && prevInventory.length > 0) {
            const candidates = prevInventory.filter((it) => {
                const t = String(it.type || '').toLowerCase();
                const r = String(it.rarity || '').toLowerCase();
                return t === 'artifact' || ["D", 'mythical', "B"].includes(r) || it.uniqueProperties?.soulbound === true;
            });
            candidates.sort((a, b) => (rarityOrder[String(b.rarity || "H").toLowerCase()] || 0) - (rarityOrder[String(a.rarity || "H").toLowerCase()] || 0));
            carried = candidates.slice(0, carryCount).map((it) => JSON.parse(JSON.stringify(it)));
        }
        ...
        set(state => ({
            player: {
                ...
                previousLives: Array.isArray(state.player.previousLives) ? state.player.previousLives : [],
                settings: {},
                cultivationDaysAllocated: 0,
                factionStanding: {},
                sectReputations: {}
        ...
        try {
            ...
            const carriedIds = Array.isArray(carried) ? carried.map((it) => it.id || it.itemId || it.name) : [];
            ...
            set(s => ({ player: { ...s.player, previousLives: [...(Array.isArray(s.player.previousLives) ? s.player.previousLives : []), entry] } }));
            // Create lightweight legacy remnants so future eras may contain traces of this life
            try {
                const legacySys = gs.legacySystem || get().legacySystem;
                if (legacySys && typeof legacySys.addRemnant === 'function') {
                    ...
                    const prevCount = (Array.isArray(gs.player.previousLives) ? gs.player.previousLives.length : 0);
                    if (prevCount > 0) {
                        legacySys.addRemnant({
        ...
        const carryMsg = (carried && carried.length > 0) ? ` Carryover: ${carried.map((x) => x.name || x.id).join(', ')}.` : '';
    ...
    reincarnateSameEra: () => {
        try {
            const gs = get();
            const inInv = Boolean(gs.ui?._inInventoryInteraction);
            ...
            if (inInv) {
                try {
                    const log = [...(gs.eventLog || []), 'Reincarnation (same era) prevented during inventory interaction (debug).'];
                    set((_s) => ({ eventLog: log }));
                }
                catch { /* ignore */ }
                return;
            }
            eraManager_1.eraManager.attemptReincarnation(gs, { sameEra: true });
            ...
            get().showToast?.(`Reincarnation failed: ${e.message || e}`, 3000, 'error');
        }
    },
    reincarnateNextEra: () => {
        try {
            ...
            if (inInv) {
                try {
                    const log = [...(gs.eventLog || []), 'Reincarnation (next era) prevented during inventory interaction (debug).'];
                    set((_s) => ({ eventLog: log }));
                }
                catch { /* ignore */ }
                return;
            }
            const prev = gs.world.currentEraIndex ?? 0;
            eraManager_1.eraManager.attemptReincarnation(gs, { nextEra: true });
            set(s => ({ world: { ...s.world } }));
            const s = get();
    ...
    reincarnateRandomFuture: () => {
        try {
            ...
            if (inInv) {
                try {
                    const log = [...(gs.eventLog || []), "Reincarnation (random future) prevented during inventory interaction (debug)."];
                    set((_s) => ({ eventLog: log }));
                }
                catch { /* ignore */ }
                return;
            ...
            const prev = gs.world.currentEraIndex ?? 0;
            eraManager_1.eraManager.attemptReincarnation(gs, { randomFuture: true });
            set(s => ({ world: { ...s.world } }));
            const s = get();
    ...
    eraDev: {
        reseedCurrent: (seed) => {
            try {
                const gs = exports.useGameStore.getState();
                ...
                exports.useGameStore.setState({ world: { ...gs.world } });
        ...
        forceNext: () => {
            ...
            try {
                ...
                if (!eraMgr)
                    return;
                eraMgr.attemptReincarnation(gs, { nextEra: true });
                exports.useGameStore.setState({ world: { ...gs.world } });
        ...
        dump: () => {
            ...
            try {
                gs.addEventLog?.('Era dump: ' + JSON.stringify(out));
            }
            catch (e) {
                void e;
            }
    ...
    workJob: () => {
        ...
        const job = jobs[Math.floor((0, seededRng_1.runtimeRng)() * jobs.length)];
        ...
        if ((0, seededRng_1.runtimeRng)() < 0.05) { // 5% chance
            const manuals = manuals_1.ALL_MANUALS.filter(m => m.rank === "H" || m.rank === "G");
            if (manuals.length > 0) {
                const foundManual = manuals[Math.floor((0, seededRng_1.runtimeRng)() * manuals.length)];
        ...
        try {
            get()._callOnPlayerActionWithGu?.('workJob');
        }
        catch { /* ignore */ }
    },
    ...
    advanceYear: () => {
        try {
            ...
            set(state => ({ player: { ...state.player, age: newAge, lastDailyReset: Date.now() }, ui: (isDead ? { ...state.ui, currentScreen: 'death' } : state.ui) }));
            // Record life event
            try {
                sharedLifePhaseSystem.recordEvent({ id: 'year_passed', title: 'A Year Passes', description: `You grow one year older to ${newAge}.`, context: { age: newAge } });
                set(state => ({ systems: { ...state.systems, lifePhase: sharedLifePhaseSystem.getState() } }));
            }
            catch (e) { /* ignore */ }
            if (isDead)
                get().addEventLog(`You have died of old age at age ${newAge}.`);
            return true;
        }
        catch (e) {
            return false;
    ...
    requestSectMission: () => {
        ...
        try {
            const mission = (0, SectMissionSystem_1.generateSectMission)(store.player);
            if (mission) {
                set(state => ({ story: { ...state.story, activeRandomMissions: [...state.story.activeRandomMissions, mission] } }));
                store.addEventLog(`Sect mission received: ${mission.title}`);
            }
        }
        catch {
    ...
    acceptSectMission: (missionId) => {
        const store = get();
        try {
            const mission = store.story?.activeRandomMissions?.find((m) => m.id === missionId);
            if (!mission)
                return false;
            // If mission has explicit rewards, try to apply them; otherwise fall back to generic rewards
            const rewards = mission.reward || { yuan: 10, spiritStones: { low: 5 } };
            // Apply simple currency / spirit stone rewards
            const newPlayer = { ...store.player };
            if (rewards.yuan)
                newPlayer.yuan = (newPlayer.yuan || 0) + (rewards.yuan || 0);
            ...
            try {
                store.addEventLog?.('Failed to accept sect mission.');
            }
            catch (e) { /* ignore */ }
            return false;
        }
    ...
    claimMissionRewards: (missionId) => {
        ...
        try {
            ...
            if (typeof rewards.yuan === 'number')
                newPlayer.yuan = (newPlayer.yuan || 0) + rewards.yuan;
            if (rewards.spiritStones) {
                newPlayer.spiritStones = {
            ...
            if (Array.isArray(rewards.items) && rewards.items.length > 0) {
                rewards.items.forEach((it) => {
                    try { /* prefer store.addToInventory to preserve side-effects */
                        store.addToInventory?.(it);
                    }
                    catch (e) { /* ignore */ }
                });
                ...
                if (chained) {
                    ...
                    try {
                        if (typeof chained === 'string') {
                            const ms = get().missionSystem || null;
                            const gameState = get();
                            let newMission = null;
                            try {
                            ...
                            if (!newMission && ms && typeof ms.generateRandomMission === 'function') {
                                try {
                                    newMission = ms.generateRandomMission(gameState);
                                }
                                catch (e) {
                                    newMission = null;
                                }
                            ...
                            if (newMission)
                                set(state => ({ story: { ...state.story, activeRandomMissions: [...state.story.activeRandomMissions, newMission] } }));
                        }
                        else if (typeof chained === 'object') {
                            set(state => ({ story: { ...state.story, activeRandomMissions: [...state.story.activeRandomMissions, chained] } }));
                        }
                    }
    ...
    checkMissionExpirations: (nowTick) => {
        ...
        try {
            ...
            set(state => {
                ...
                return { story: { ...state.story, activeRandomMissions: remaining } };
            });
            for (const m of expired) {
                try {
                    store.addEventLog?.(`Mission expired: ${m.title || m.id}`);
                }
                catch (e) { /* ignore */ }
            }
    ...
    updateMissionObjectiveProgress: (missionId, objectiveIndex, delta = 1) => {
        const store = get();
        try {
            set(state => {
                ...
                return { story: { ...state.story, activeRandomMissions: missions } };
            });
            return true;
        }
        catch (e) {
    ...
    getRivalById: (id) => sharedRivalSystem.getRival(id),
    startRivalEncounter: (rivalId) => {
        const store = get();
    ...
    startCombatWithRival: (rivalId) => {
        ...
        try {
            ...
            const cs = new CombatSystem_1.CombatSystem(player, [enemy], get(), sharedRivalSystem, { type: 'rival', rivalId });
            set(state => ({ combatSystem: cs, ui: { ...state.ui, currentScreen: 'combat' } }));
            get().addEventLog(`Encountered rival ${sharedRivalSystem.getRival(rivalId)?.name}.`);
            return true;
        }
        catch {
            return false;
    ...
    learnManualById: (manualId) => {
        const store = get();
        const manual = manuals_1.ALL_MANUALS.find(m => m.id === manualId);
        if (!manual)
            return false;
        if (store.player.manuals.some(m => m.id === manualId))
            return false;
        set(state => ({ player: { ...state.player, manuals: [...state.player.manuals, manual] } }));
        get().addEventLog(`Learned manual: ${manual.name}`);
        return true;
    },
    attemptStudy: (manualId) => {
        const store = get();
        const manual = manuals_1.ALL_MANUALS.find(m => m.id === manualId);
        if (!manual)
            return false;
        ...
        // Check resources
        if (store.player.yuan < yuanCost || store.player.currentQi < qiCost)
            return false;
        ...
        set(state => {
            ...
            if (manual.effects?.stats) {
                p.stats = { ...p.stats };
                Object.entries(manual.effects.stats).forEach(([k, v]) => {
                    if (typeof v === 'number')
                        p.stats[k] = (p.stats[k] || 0) + v;
                    else if (v && typeof v === 'object' && typeof v.base === 'number')
                        p.stats[k] = (p.stats[k] || 0) + v.base;
                });
            }
            if (manual.effects?.skills) {
                p.skills = { ...p.skills };
                Object.entries(manual.effects.skills).forEach(([sid, amt]) => {
                    if (!p.skills[sid])
                        p.skills[sid] = { level: 0, exp: 0, expToNext: 100 };
    ...
    canEvolveManual: (_manualId) => ({ can: false, unmet: [] }),
    evolveManual: (_manualId) => false,
    ...
    getRivalEncounterStatus: (rivalId) => {
        const store = get();
    ...
    getNextRivalEncounterInDays: (rivalId) => {
        const store = get();
        const rival = sharedRivalSystem.getRival(rivalId);
        if (!rival)
            return 999;
    ...
    getRivalFactionStanding: (rivalId) => {
    ...
    joinSect: (sectId) => {
        const store = get();
        if (store.player.sect)
            return false; // already in a sect
        set(state => ({
            player: {
                ...state.player,
                sect: sectId,
    ...
    adjustSectReputation: (sectId, amount) => {
        set(state => {
            ...
            const sys = { ...(state.systems.sectReputations || {}) };
            const ply = { ...(state.player.sectReputations || {}) };
    ...
    adjustFactionStanding: (factionId, amount) => {
        set(state => {
            const clamp = (v) => Math.max(-100, Math.min(100, v));
            const map = { ...(state.systems.factionStandings || {}) };
            map[factionId] = clamp((map[factionId] || 0) + amount);
            return { systems: { ...state.systems, factionStandings: map } };
        });
    },
    getFactionStanding: (_factionId) => {
        const map = get().systems.factionStandings || {};
        return map[_factionId] || 0;
    },
    getAvailableFactionServices: (_factionId) => [],
    canAccessFactionService: (_factionId, _serviceId) => ({ canAccess: false, reason: '' }),
    purchaseFactionService: (_factionId, _serviceId) => false,
    triggerFactionBattleEvent: (_playerFaction, _enemyFaction) => ({ success: false, outcome: 'defeat', standingChange: 0, enemyStandingChange: 0 }),
    ...
    getActivePhysiqueSynergies: () => {
        ...
        return (0, physiqueSynergies_1.getActiveSynergies)(activePhysiques);
    },
    getPhysiqueSynergyBonuses: () => {
        const store = get();
        const activePhysiques = store.player.physique ? [store.player.physique] : [];
        return (0, physiqueSynergies_1.calculateSynergyBonuses)(activePhysiques);
    }
}));
...
try {
    ...
    let lastTick = exports.useGameStore.getState().world.tick || 0;
    exports.useGameStore.subscribe((state) => state.world.tick, (tick) => {
        try {
            if (typeof tick === 'number' && tick !== lastTick) {
                lastTick = tick;
                try {
                    exports.useGameStore.getState().processTravelTick();
                }
                catch (e) { /* ignore */ }
            }
        }
...
try {
    const store = exports.useGameStore.getState();
    ...
    if (store && store.lifePhaseSystem && typeof store.lifePhaseSystem.addPhaseEndListener === 'function') {
        store.lifePhaseSystem.addPhaseEndListener((phase) => {
            try {
                ...
                if (phase && phase.type && (String(phase.type).toLowerCase().includes('ascend') || String(phase.type).toLowerCase().includes('immortal') || phase.title && phase.title.toLowerCase().includes('ascension'))) {
                    exports.useGameStore.setState(state => ({ world: { ...state.world, ascended: true, currentWorldType: 'immortal', worldTier: Math.max(1, (state.world.worldTier || 1) + 1) } }));
                }
                // Phase-end may also set world-level flags if the phase had karmicModifiers or events
                if (phase && phase.karmicModifiers) {
                    exports.useGameStore.setState(state => ({ world: { ...state.world, flags: { ...(state.world.flags || {}), ...phase.karmicModifiers } } }));
                }
            }
            catch (e) { /* non-fatal */ }
        });
    ...
    if (!store.player.equipment) {
        exports.useGameStore.setState((state) => ({ player: { ...state.player, equipment: { mainHand: null, offHand: null, armor: null, accessory1: null, accessory2: null, mount: null, companion: null, innerCore: null } } }));
    }
    // Expose helper functions on the store instance (runtime) for quick scripting access
    exports.useGameStore.equipItem = (slot, item) => {
        exports.useGameStore.setState((s) => {
            const prev = (s.player.equipment || {})[slot];
            let player = { ...s.player };
            // remove previous bonuses
            player = EquipmentSystem_1.default.removeEquipmentBonuses(player, prev);
            player = EquipmentSystem_1.default.applyEquipmentBonuses(player, item);
            return { player: { ...player, equipment: { ...(s.player.equipment || {}), [slot]: item } } };
        });
    };
    exports.useGameStore.unequipItem = (slot) => {
        exports.useGameStore.setState((s) => {
            const prev = (s.player.equipment || {})[slot];
            let player = { ...s.player };
            player = EquipmentSystem_1.default.removeEquipmentBonuses(player, prev);
