...
describe('Cultivation minor stage counts (authoritative)', () => {
    test('key realms have expected minor stage counts', () => {
        ...
        for (const [realm, count] of Object.entries(expected)) {
            ...
            expect((0, cultivationRealms_1.getMinorStageCount)(realm)).toBe(count);
            expect(cultivationRealms_1.CULTIVATION_REALMS[realm]).toBeDefined();
            expect(cultivationRealms_1.CULTIVATION_REALMS[realm].minorStages).toBe(count);
        }
    });
});
