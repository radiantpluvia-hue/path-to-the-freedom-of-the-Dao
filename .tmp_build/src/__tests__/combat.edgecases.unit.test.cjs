...
describe('CombatSystem edge cases: multiHit, chain/on_kill, diminishing returns, mastery cooldown reduction', () => {
    test('multiHit mechanic applies multiple hits and stops if target dies mid-sequence', () => {
        const attacker = { id: 'att', name: 'A', hp: 50, maxHp: 50, qi: 10, maxQi: 10, ap: 5, maxAp: 5, stats: { atk: 5, def: 0, speed: 3 }, techniques: [], buffs: [], debuffs: [] };
        const target = { id: 't', name: 'T', hp: 8, maxHp: 8, qi: 0, maxQi: 0, ap: 5, maxAp: 5, stats: { atk: 1, def: 0, speed: 2 }, techniques: [], buffs: [], debuffs: [] };
        // Technique with multiHit 3 and base damage 4 -> should kill target on first hit and not attempt further hits
        const tech = { id: 'mh', name: 'MH', apCost: 0, qiCost: 0, type: 'attack', effects: [{ type: 'damage', target: 'enemy', value: 4 }], mechanics: [{ type: 'multiHit', hits: 3 }] };
    ...
    test('chain on_kill spills damage to other enemies only when kill occurs', () => {
        const attacker = { id: 'att', name: 'A', hp: 50, maxHp: 50, qi: 10, maxQi: 10, ap: 5, maxAp: 5, stats: { atk: 6, def: 0, speed: 3 }, techniques: [], buffs: [], debuffs: [] };
        const target1 = { id: 't1', name: 'T1', hp: 4, maxHp: 4, qi: 0, maxQi: 0, ap: 5, maxAp: 5, stats: { atk: 1, def: 0, speed: 2 }, techniques: [], buffs: [], debuffs: [] };
        const target2 = { id: 't2', name: 'T2', hp: 20, maxHp: 20, qi: 0, maxQi: 0, ap: 5, maxAp: 5, stats: { atk: 2, def: 0, speed: 2 }, techniques: [], buffs: [], debuffs: [] };
        // Chain with on_kill semantics implemented as conditional.type='conditional' and condition 'on_kill'
        const tech = { id: 'ck', name: 'CK', apCost: 0, qiCost: 0, type: 'attack', effects: [{ type: 'damage', target: 'enemy', value: 6 }], mechanics: [{ type: 'conditional', condition: 'on_kill', multiplier: 0.5 }] };
        attacker.techniques = [tech];
        const cs = new CombatSystem_1.CombatSystem(attacker, [target1, target2], null, null, { type: 'normal', rng: () => 0.5 });
    ...
    test('diminishing returns reduces damage on repeated uses of same technique', () => {
        const attacker = { id: 'att', name: 'A', hp: 200, maxHp: 200, qi: 20, maxQi: 20, ap: 5, maxAp: 5, stats: { atk: 10, def: 0, speed: 3 }, techniques: [], buffs: [], debuffs: [] };
        const target = { id: 't', name: 'T', hp: 200, maxHp: 200, qi: 0, maxQi: 0, ap: 5, maxAp: 5, stats: { atk: 2, def: 0, speed: 2 }, techniques: [], buffs: [], debuffs: [] };
        const tech = { id: 'dim', name: 'Dim', apCost: 0, qiCost: 0, type: 'attack', effects: [{ type: 'damage', target: 'enemy', value: 20 }], mechanics: [] };
        attacker.techniques = [tech];
        const cs = new CombatSystem_1.CombatSystem(attacker, [target], null, null, { type: 'normal', rng: () => 0.5 });
    ...
    test('mastery rank reduces cooldown when technique used repeatedly', () => {
        const attacker = { id: 'att', name: 'A', hp: 50, maxHp: 50, qi: 20, maxQi: 20, ap: 5, maxAp: 5, stats: { atk: 8, def: 0, speed: 3 }, techniques: [], buffs: [], debuffs: [] };
        const target = { id: 't', name: 'T', hp: 100, maxHp: 100, qi: 0, maxQi: 0, ap: 5, maxAp: 5, stats: { atk: 2, def: 0, speed: 2 }, techniques: [], buffs: [], debuffs: [] };
        const tech = { id: 'mst', name: 'MasteryTech', apCost: 0, qiCost: 0, type: 'attack', cooldown: 4, effects: [{ type: 'damage', target: 'enemy', value: 6 }], masteryXp: 0, masteryRank: 0 };
        attacker.techniques = [tech];
        ...
        for (let i = 0; i < 6; i++) {
            ...
            if (attacker.techniques[0].currentCooldown && attacker.techniques[0].currentCooldown > 0)
