...
jest.mock('@/store/useGameStore', () => {
    ...
    mockState.startCultivation = jest.fn((minutes) => { mockState.ui.isCultivating = true; });
    ...
    mockState.getQuestsByType = jest.fn((type) => []);
...
test('Start/Stop cultivation buttons call store APIs and Attempt Breakthrough calls handler', () => {
    ...
    const quickButtons = react_1.screen.getAllByText(/Quick Cultivate|Cultivate/i);
    ...
    const smallBtn = react_1.screen.getByRole('button', { name: /cultivate/i });
    react_1.fireEvent.click(smallBtn);
