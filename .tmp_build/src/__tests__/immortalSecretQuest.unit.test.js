...
describe('Immortal secret quest', () => {
    test('fails when no clue found', () => {
        const r = (0, immortalSecretQuest_1.attemptSecretQuest)({ playerReputation: 100, foundSecretClue: false });
        expect(r.success).toBe(false);
    });
    test('succeeds when clue found and stealth success', () => {
        const rng = seededRng(12345);
        const r = (0, immortalSecretQuest_1.attemptSecretQuest)({ playerReputation: 0, foundSecretClue: true, stealthSuccessful: true }, rng);
    ...
    test('deterministic selection uses spawnWeight with seeded RNG', () => {
        ...
        const r = (0, immortalSecretQuest_1.attemptSecretQuest)({ playerReputation: 100, foundSecretClue: true }, rng);
        expect(r.success).toBe(true);
