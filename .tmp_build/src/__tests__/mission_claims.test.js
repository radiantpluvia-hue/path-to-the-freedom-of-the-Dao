...
describe('Mission claim rewards', () => {
    ...
    test('claimMissionRewards applies rewards and moves mission to completed', () => {
        ...
        expect(useGameStoreMock.getState().player.inventory.some((i) => i.id === 'test_token')).toBe(true);
        expect(useGameStoreMock.getState().story.completedQuests).toContain('m_test_1');
        expect(useGameStoreMock.getState().story.activeRandomMissions.find((m) => m.id === 'm_test_1')).toBeUndefined();
    });
