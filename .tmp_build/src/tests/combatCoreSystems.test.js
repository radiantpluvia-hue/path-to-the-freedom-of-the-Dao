...
const makeParticipant = (id, name, stats, extra) => ({
    id,
    name,
    hp: 100,
    maxHp: 100,
...
const makeAttack = (id = 'hit', value = 0, over) => ({
    id,
    name: id,
...
const makeBuff = (id = 'atk_buff', stat = 'atk', value = 2, duration = 1) => ({
    id,
    name: id,
    description: 'buff',
    apCost: 0,
...
const systemWith = (player, enemy, ctx) => new CombatSystem_1.CombatSystem(player, [enemy], undefined, undefined, { type: 'normal', ...ctx });
describe('Combat core systems', () => {
    test('environment multipliers adjust damage (storm > clear)', () => {
        ...
        // Clear
        const p1 = { ...basePlayer, techniques: [strike] };
        const e1 = { ...baseEnemy };
    ...
    test('getAvailableTechniques respects movement stance AP discount', () => {
        ...
        const enemy = makeParticipant('e', 'Dummy', { atk: 0, def: 0, speed: 10 });
        const costly = { id: 'dash_slash', name: 'Dash Slash', description: '', apCost: 2, qiCost: 0, type: 'attack', effects: [{ type: 'damage', target: 'enemy', value: 1 }], cooldown: 0, currentCooldown: 0 };
