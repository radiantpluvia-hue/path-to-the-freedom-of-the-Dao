...
exports.exportChronicleToMarkdown = exportChronicleToMarkdown;
...
function exportChronicleToMarkdown(save) {
    ...
    if (snapshot?.phases && Array.isArray(snapshot.phases) && snapshot.phases.length > 0) {
        ...
        snapshot.phases.forEach((phase, idx) => {
            ...
            if (Array.isArray(phase.events) && phase.events.length > 0) {
                ...
                phase.entries.forEach((e) => md += `- ${String(e)}\n`);
                md += '\n';
            }
            else if (phase.note) {
                md += `${phase.note}\n\n`;
...
exports.default = exportChronicleToMarkdown;
