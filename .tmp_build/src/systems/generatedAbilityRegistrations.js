...
exports.enrichGeneratedAbilities = enrichGeneratedAbilities;
async function enrichGeneratedAbilities() {
    const mod = await Promise.resolve().then(() => __importStar(require('../data/generated/activeAbilities.generated')));
    ...
    for (let idx = 0; idx < GENERATED.length; idx++) {
        ...
        if (n % 11 === 0)
            a.mechanics.push({ type: 'multiHit', hits: 2 });
        if (n % 13 === 0)
            a.mechanics.push({ type: 'chain', chainChance: 0.2 });
        if (n % 17 === 0)
            a.mechanics.push({ type: 'conditional', onKill: { healSelf: Math.max(3, Math.floor(n % 10)) } });
        if (typeof a.power !== 'number')
            a.power = a.effects && a.effects[0] ? a.effects[0].value : 5;
