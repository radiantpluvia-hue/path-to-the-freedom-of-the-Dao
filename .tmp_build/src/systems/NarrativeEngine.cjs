...
class NarrativeEngine {
    ...
    generateEventsForPlayer(context) {
        ...
        for (const t of this.narrativeTriggers) {
            ...
            for (const [key, expected] of Object.entries(conds)) {
                ...
                if (!found && context.player && key in context.player) {
                    if (expected === undefined)
                        ...
                        found = context.player[key] === expected;
                }
                // check world
                if (!found && context.world && key in context.world) {
                    if (expected === undefined)
                        ...
                        found = context.world[key] === expected;
                }
                // check story
                if (!found && context.story && key in context.story) {
                    if (expected === undefined)
                        ...
                        found = context.story[key] === expected;
                }
                // check ui
            ...
            if (matches) {
                ...
                if (t.affinityBias === 'positive' && playerAffinity <= 0)
                    continue;
                if (t.affinityBias === 'negative' && playerAffinity >= 0)
                    continue;
                    ...
                    if (destinySystem && typeof destinySystem.getInfluenceableThreadsForPlayer === 'function') {
                        try {
                            ...
                            for (const temp of templates) {
                                ...
                                if (score > 0)
                                    prioritized.push({ temp, score });
                                else
                                    others.push({ temp, score: 0 });
                            }
