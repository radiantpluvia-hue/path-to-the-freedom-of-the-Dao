...
exports.market = exports.MarketSystem = exports.MARKETS = exports.MARKET_ITEMS = void 0;
const registry_1 = require("../data/registry");
const logger_1 = require("../utils/logger");
const relicReg = __importStar(require("./relicRegistry"));
...
exports.MARKETS = [
    {
        ...
        type: 'general',
        items: exports.MARKET_ITEMS.filter(item => item.rarity === "H" ||
    ...
    },
    {
        ...
        type: 'immortal',
        items: exports.MARKET_ITEMS.filter(item => item.rarity === "F" || item.rarity === "E" || item.rarity === "D"),
...
class MarketSystem {
    ...
    getAvailableMarkets(playerState) {
        return this.markets.filter(market => {
            if (!market.requirements)
                return true;
            if (market.requirements.minRealm && ((0, playerHelpers_1.getPlayerRealmId)(playerState) || 0) < market.requirements.minRealm) {
                return false;
            }
    ...
    getMarketItems(marketId, playerState) {
        ...
        return market.items.filter(item => {
            if (!item.requirements)
                return true;
            if (item.requirements.minRealm && ((0, playerHelpers_1.getPlayerRealmId)(playerState) || 0) < item.requirements.minRealm) {
                return false;
            }
    ...
    buyItem(marketId, itemId, playerState) {
        ...
        try {
            if (String(item.id || '').startsWith('relic_')) {
                ...
                if (relicReg && typeof relicReg.claimRelic === 'function')
                    relicReg.claimRelic(rid);
            }
        }
        catch (e) { /* ignore */ }
        // Reduce stock
        ...
        try {
            if (this.rng() < 0.002) {
                try {
                    ...
                    const allRelics = registry_1.getRelics && typeof registry_1.getRelics === 'function' ? (0, registry_1.getRelics)() : [];
                    ...
                    if (sampledRelic) {
                        const relicItem = {
                            ...
                            type: 'treasure',
                            rarity: (sampledRelic.rarity || 'mythical'),
                            price: { yuan: 50000 },
    ...
    getActiveAuctions() {
        try {
            ...
            try {
                logger_1.logger.warn('MarketSystem.getActiveAuctions failed:', e);
            }
            catch { /* ignore */ }
            return [];
        }
        ...
        try {
            this.activeAuctions.forEach(auction => {
                ...
                if (auction.timeRemaining <= 0 && auction.currentBidder === 'player') {
                    ...
                    catch (inner) { /* ignore inventory failure */ }
            ...
            try {
                logger_1.logger.warn('MarketSystem.updateAuctionTimes failed:', e);
            }
            catch { /* ignore */ }
    ...
    generateRandomAuctions() {
        ...
        for (let i = 0; i < auctionCount; i++) {
            const idx = Math.floor(this.rng() * exports.MARKET_ITEMS.length);
            const randomItem = exports.MARKET_ITEMS[idx];
            if (!randomItem) {
    ...
    async enchantItemForPlayer(targetItemId, gemItemId, gsOrPlayer) {
        try {
            const mod = await Promise.resolve().then(() => __importStar(require('./EnchantingSystem')));
            const shared = (mod && (mod.sharedEnchantingSystem || mod.sharedEnchantingSystem)) || null;
            if (!shared || typeof shared.socketItem !== 'function')
                return false;
...
exports.market = new MarketSystem();
