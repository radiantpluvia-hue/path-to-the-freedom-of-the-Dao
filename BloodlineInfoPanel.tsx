...
export function BloodlineInfoPanel({ bloodline }: BloodlineInfoPanelProps) {
  return (
    <div style={{ fontSize: '0.9rem' }}>
      ...
      {bloodline.effects.stats && (
        <div style={{ marginBottom: '10px' }}>
          <strong>Stat Bonuses:</strong>
          <ul style={{ margin: '5px 0', paddingLeft: '15px' }}>
            {Object.entries(bloodline.effects.stats).map(([stat, value]) => (
      ...
      {bloodline.effects.skills && (
        <div style={{ marginBottom: '10px' }}>
          <strong>Skill Bonuses:</strong>
          <ul style={{ margin: '5px 0', paddingLeft: '15px' }}>
            {Object.entries(bloodline.effects.skills).map(([skill, value]) => (
              <li key={skill} style={{ fontSize: '0.85rem' }}>
                {skill}: +{value}
              </li>
            ))}
          </ul>
